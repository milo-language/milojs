...
// bun's own published table, captured by tools/gen-bun-claims.mjs. Asserted, not
// measured, and that is exactly why it is here: printed beside the measurement
// of the same bun binary on the same files, the gap between "implemented" and
// "node's cases pass" stops being something a reader has to rediscover.
const claimsPath = join(ROOT, "docs/conformance/bun-claims.json");
const claims = existsSync(claimsPath) ? JSON.parse(readFileSync(claimsPath, "utf-8")) : null;
...
// A module bun's page does not list at all is blank, not red: the page simply
// says nothing about it, which is a different statement from "not implemented".
const claimCell = (c) => (c && CLAIM_DOT[c] ? CLAIM_DOT[c] : (c === "unknown" ? "?" : ""));
...
lines.push("");
lines.push("The dot is the pass rate of node's own `test-<area>-*.js` cases, passed out of RAN; skipped cases score neither way.");
lines.push(`🟢 90%+ \u00b7 🟡 60%+ \u00b7 🟠 30%+ \u00b7 🔴 below \u00b7 ⚪ nothing ran. A module that does not load is 🔴 whatever its cases did, and under ${MIN_CASES} cases that ran the ceiling is 🟠.`);
lines.push("");
lines.push("Exports do not affect the dot: that column is the worklist, not a score.");
if (peerLabel) {
  lines.push("");
  lines.push(`The ${peerLabel} column is this repo's own sweep over the identical files with \`MILOJS_RUNTIME=$(which bun)\`. The \"bun says\" column is what bun's published table claims for the same module. One is measured, the other is asserted, and they sit side by side on purpose: bun's 🟢 means the module is implemented, the dot here means node's cases pass.`);
  lines.push("");
  lines.push("Compare the two cells only through their skip counts: a smaller denominator flatters the rate. The whole-suite line under the table is the comparable form.");
}
lines.push("");
lines.push(`Reference surface: node ${snapshot.node.version}` +
  (report ? `. Sweep at \`${(report.milojs?.revision ?? "").slice(0, 8)}\`` : "") + ".");
lines.push("");
lines.push(`| | module | tests |${peerLabel ? ` ${peerLabel} |` : ""}${claims ? " bun says |" : ""} exports | missing exports |`);
lines.push(`|---|---|---|${peerLabel ? "---|" : ""}${claims ? "---|" : ""}---|---|`);
...
// The cross-check, DERIVED. Every number here is computed from the same two
// files the table is: nothing about it is maintained by hand, so it cannot drift
// away from the measurement the way the claim it is checking does.
if (claims && peer) {
  const claimedFull = rows.filter((r) => r.claim === "full");
  const measured = claimedFull.reduce((a, r) => ({
    pass: a.pass + (r.peer?.pass ?? 0),
    ran: a.ran + (r.peer?.ran ?? 0),
  }), { pass: 0, ran: 0 });
  const greenHere = claimedFull.filter((r) => dot({ loads: true, tests: r.peer }) === "\u{1F7E2}").length;
  const weBeat = claimedFull.filter((r) =>
    testPct(r) !== null && r.peer?.ran > 0 && testPct(r) > (r.peer.pass / r.peer.ran) * 100).length;
  lines.push("");
  lines.push(
    `Cross-check against bun's published table (\`docs/conformance/bun-claims.json\`, which tracks node ${claims.tracksNode ?? "unstated"}): ` +
    `bun marks **${claimedFull.length}** of these modules fully implemented. Measured on this corpus, those ${claimedFull.length} ` +
    `pass **${measured.pass}/${measured.ran} (${pct(measured.pass, measured.ran)})** and **${greenHere}** of them reach 🟢 on the scale above` +
    `${weBeat ? `, while milojs outscores bun on ${weBeat} of them` : ""}. ` +
    `That is the whole answer to "bun says green, so why are we not green": the two dots are not the same claim, and a runtime can ` +
    `ship a module as implemented while a third of node's cases for it still fail. Nothing here is a bug in bun's table; it answers a different question than this one does.`);
}
