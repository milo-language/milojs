#!/usr/bin/env node
// Generates src/engine/uniprops.txt (the data) and validates src/engine/uniprops.milo can find
...
// property.
//
//   node tools/gen-uniprops.mjs            # rewrite src/engine/uniprops.txt
...
// (the shape src/engine/unicase.milo uses) would be megabytes of generated code; as
...
const target = new URL("../src/engine/uniprops.txt", import.meta.url);
if (process.argv.includes("--check")) {
  const have = readFileSync(target, "utf8");
  if (have !== out) {
    console.error("gen-uniprops: src/engine/uniprops.txt does not match its generator.");
  ...
  console.log(`gen-uniprops: src/engine/uniprops.txt matches its generator (${lines.length} properties)`);
} else {
