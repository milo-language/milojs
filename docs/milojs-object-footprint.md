...
| | fields |
|---|---:|
| `JSObj` (the hot header) | 34 |
| `JSObjExtra` (the side table) | 26 |

The header keeps what a plain object actually touches: `props`, `elems`,
`holes`, `logicalLen`, `proto`, the boolean brand flags the dispatcher
tests on the hot path, `extra`, and the collector's `mark` and `free` bits.
`fnIdx`/`fnEnv` are -1 except on a closure's function object (every closure owns
one, see `makeClosure`), where they let the collector reach the captured scope.
Everything a plain object never becomes is one index away instead of inline.
`realm` (an `i32`, the realm the object was allocated in: a function's [[Realm]],
and the realm an implicit prototype link resolves in) sits in the four bytes of
padding after the run of twelve flags that ends at `isDataView`, so adding it
left the header's size unchanged; the LLVM type confirms the placement.
