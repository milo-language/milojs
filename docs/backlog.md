...
case, and not fine for a program calling `vm.runInNewContext` in a loop, which
`lib/vm.js` now turns into one realm per call. Freeing one needs the collector to treat a
...
## node:vm: what contexts-as-realms still leaves out

A context is a realm with its sandbox in front of the global scope
(`identTryLookup` in `src/engine/realm.milo`), and `this` inside is a Proxy
`lib/vm.js` builds over the sandbox and the realm's own global. What node's vm
tests still fail on, by cause:

- `timeout` and `breakOnSigint` are validated and not enforced: the evaluator
  has no interrupt, so `test-vm-timeout` runs its infinite loop until the sweep
  kills it at several GB. Needs a deadline check on loop back-edges and calls,
  and a termination that `catch` cannot swallow.
- `microtaskMode: "afterEvaluate"`: there is one job queue, not one per context.
- No line or column in a stack (see "Errors" below), so node's
  `filename:line` decoration of a context's error (test-vm-basic,
  test-vm-context) cannot be produced. The filename is there.
- A script's completion value counts expression statements only:
  `vm.runInContext("if (1) { 2 }")` is undefined, node says 2.
- A Proxy sandbox without a `set` trap: the engine forwards [[Set]] to the
  target without consulting the receiver's `getOwnPropertyDescriptor` trap,
  which is where node's error comes from (test-vm-set-property-proxy).
- Wording: `script.runInNewContext.call(x)` reports `runInContext is not a
  function` where node says `this.runInContext`, and a strict write through
  the context's global to a read-only sandbox property reports the proxy's
  `'set' on proxy` TypeError where node says `Cannot redefine property`.
- Every context is a whole realm that is never freed (above).
...
The same missing table shows up two more ways, both hit by node's vm tests.
`delete globalThis.x` after a sloppy `x = 1` leaves the binding in place
(test-vm-basic deletes `vmResult` and then finds it still there), and a property
DEFINED on the global object (`Object.defineProperty(globalThis, "q", { get })`)
is not a global name at all: `q` is a ReferenceError
(test-vm-context-dont-contextify). `Reflect.ownKeys(globalThis)` is empty for
the same reason: the bindings are not properties.
...
## Parser: `var = 1;` is accepted

`var = 1; console.log("parsed")` prints `parsed`; node rejects the file with
`SyntaxError: Unexpected token '='`. A `var` with no binding identifier must be
an early error, in scripts and in `eval` alike. Found while writing the realm
fixture, which spells its SyntaxError case with `(` instead because of this.
