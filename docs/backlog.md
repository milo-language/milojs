...
key-files: src/engine/eval.milo, bench/ab.sh, src/engine/builtins.milo, src/engine/parser.milo, src/engine/methods.milo, src/engine/runtime.milo, src/engine/driver.milo, src/engine/bytecode.milo, scripts/test262-sweep.ts, scripts/quickjs-sweep.ts, lib/http.js, lib/fs.js, bench/run.sh, bench/arith.js
...
stayed white) and the same for FinalizationRegistry and WeakRef, which do not
exist yet. Lookup is also a linear scan, as for Map (`mapFind`).
