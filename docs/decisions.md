...
last-verified: 2026-09-20 (re-verified after the explicit &mut call-argument migration: every bare argument bound to a &mut parameter now reads '&mut x', a spelling change only; no behaviour this doc describes changes. Previous note: re-verified after mode B: the execution-strategy entry gains no new rule; declared locals move between slot and scope backing per chunk, decided at compile. Previous note: re-verified for the ops batch: the shared-core rule now also covers in/instanceof/loose-eq, which strengthens the execution-strategy entry rather than changing it. Previous note: re-verified after Op.CallMember: the execution-strategy entry already describes the shared-core rule the opcode follows; lane and rooting entries unchanged)
-->
