...
in this tree: <!--fact:loc-milo-->51.0k<!--/fact--> lines of Milo, one 17.7k-line evaluator, and
...
stack, which is unreachable). Roots = the global scope (scope 0 then; every realm's since realms) + an explicit `active` dynamic-call-stack
...
(`funcProtos`, a GC root; superseded 2026-09-22, it is now an ordinary own property of each
closure's function object, see `makeClosure` in `src/engine/runtime.milo`); `new F()` links the instance's `proto`; `getMember` walks own-props →
