...
last-verified: 2026-09-22 (re-verified after node:vm moved onto realms: a realm can carry a sandbox (RealmIntrinsics.sandboxObj, given by __realmCreate(sandbox)); global names in such a realm resolve through identTryLookup/identAssign/identDelete, which run the sandbox's traps and accessors, declared vars and functions go through declareSandboxGlobal, code in such a realm never runs as bytecode, and __realmEval takes a filename that names the script's stack frame. Previous note: re-verified after the realm creation commit: createRealm (src/engine/realm.milo) boots a new realm through the same setupGlobals and the recorded JS preludes (runRealmPrelude), __realmCreate/__realmEval expose it, test262-sweep's $262 gains createRealm/evalScript and the sweep a --files option, and GetPrototypeFromConstructor falls back to newTarget's realm. The embedding ABI stays one context; libmilojs records its prelude through runPreludeSource so a realm made there gets the prelude too. Previous note: re-verified after the call-time realm commit: JSObj gains realm (i32, in existing padding), a call to a function, builtin or builtin method of another realm enters that realm for the call and leaves it on return or throw, a parked activation's ExecCtx carries its realm, an implicit prototype link resolves in the object's own realm, and the instanceof/species shortcuts answer only within one realm. One realm still exists, so no behaviour this doc describes changes. Previous note: re-verified after the realm commits recompiled status.md's line-count fact; the publication rule and report shapes are unchanged. Previous note: re-verified after the builtin function-object commit: JSValue.Native carries a second field, the index of a real JSObj that is the builtin's identity and property bag; one canonical object per Builtin and typed-array kind lives in Interp.nativeObjs (nativeValue is the only way to make such a value), a promise's resolve and reject functions get a fresh one each (makeResolver, Native.Resolve/Reject), and Interp.nativeKeys/nativeProps/resolverProps are gone. Previous note: re-verified with status.md after the sweep argument check. Previous note: re-verified after setMember folded its builtin-prototype invalidation into touchedBuiltinProto and the sweeps began refusing unknown arguments; nothing this doc describes changes. Previous note: re-verified after the typed-array dispatch commit, which rewrites gate 2's prose and drops the bigint64-from known limit from docs/status.md; the reports, their schema and the publication rule are unchanged. Previous note: re-verified after the DataView dispatch commit, which rewrites gate 2's prose and recompiles the fixture facts in docs/status.md; the reports, their schema and the publication rule are unchanged. Previous note: re-verified after the RegExp dispatch commit, which rewrites gate 2's prose and drops the regexp-symbols known limit from docs/status.md; the reports, their schema and the publication rule are unchanged. Previous note: re-verified after the Date dispatch commit, which rewrites gate 2's prose and recompiles the fixture facts in docs/status.md; the reports, their schema and the publication rule are unchanged. Previous note: re-verified after the WeakMap/WeakSet dispatch commit, which rewrites gate 2's prose and recompiles the fixture and line-count facts in docs/status.md; the reports, their schema and the publication rule are unchanged. Previous note: re-verified after the symbol primitive commit, which recompiles the fixture and line-count facts in docs/status.md; the reports, their schema and the publication rule are untouched. Previous note: re-verified after the closure function-object commit, which recompiles the fixture and line-count facts in docs/status.md; the reports, their schema and the publication rule are untouched. Previous note: re-verified after the scoped-temp-root commit and the gc-stress gate; the reports and their schema are untouched. Previous note: re-verified after the doc-facts recompile commit 4ea448d and the class heritage commit; nothing about the reports changes. Previous note: re-verified after the ToNumber(Symbol) commit (toNumProg throws for a symbol, toNumArg retired, numArgsProg feeds Math/isNaN/Date setters/Number methods, toStrArg for parseInt and string arguments); nothing this doc describes changes. Previous note: re-verified after the import-list commit: every name a file used without importing it is now listed in its import block (the milo compiler enforces the list from 2026-09-20; scripts/fix-imports.ts there added them), a spelling change only; nothing this doc describes changes. Previous note: re-verified after top-level `this` and global-descriptor commit (unbound this answers the global object, CommonJS binds this to exports, markBuiltinGlobals fixes the builtin/var boundary for globalThis descriptors) and the JSON.stringify fast path; nothing this doc describes changes. Previous note: re-verified after the 78b8043 report commit (1189) and the descriptor-semantics commit; schema unchanged. Previous note: re-verified after test262-sweep.ts began staging a case directory's _FIXTURE.js files beside case.js; the report schema and the numbers it cites are unchanged, the sample gained three dynamic-import passes. Previous note: re-verified after the map/set dispatch commit: status.md moved only by its recompiled fixture and line counts; nothing about producing a report changed. Previous note: re-verified after the timer unref and mkdir errno commits: status.md moved only by its recompiled fixture count; nothing about producing a report changed. Previous note: re-verified after the node pass-set claim: report production unchanged, the pass set holds 584. Previous note: re-verified after mode B; production unchanged. Previous note: re-verified for the ops batch; production unchanged, coverage floor now 58.3. Previous note: re-verified after op.callmember and the coverage-floor ratchet: report production unchanged; vm-coverage floor now 53.6. Previous note: re-verified: report production unchanged by the raw-f64 lane and the node-compat pass-set seeding; the passes array now exists in all three suite reports. Previous note: vm-coverage.json joins the committed reports: produced by tools/vm-coverage.mjs via milojs-engine --vm-audit, floored by tools/check-vm-coverage.mjs; sweep report production itself is unchanged. Previous note: reports now carry a per-case passes array, which seeds docs/conformance/passset-*.txt via tools/gen-passset.mjs and is what tools/check-conformance-ratchet.mjs holds; producing a report is unchanged. Previous note: re-read when the constant tables were rekeyed per release target, which moved the `constants` row in docs/node-compat.md and the JS line count in docs/status.md; the report schema, the procedure and the publication rule are unchanged; re-read again after the linux-x64 and linux-arm64 tables were harvested from CI runners, which moved the same two lines)
-->
...
`--files <list>` scores exactly the cases a newline-separated file names (paths
relative to the corpus's `test/`, or absolute), for a subset no directory
selects: every case that calls `$262.createRealm`, for instance, is
`grep -rl createRealm $TEST262/test` minus intl402 and staging, which the sweep
never walks. Like `--dir`, it cannot write the committed report. The sweep's
`$262` gives `createRealm()` and `evalScript(src)` through the engine intrinsics
`__realmCreate` and `__realmEval`; see `HOST_HOOK` in the script.
