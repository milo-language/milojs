...
int main(void) {
    ...
    if (eval(first, "function (", &value) != MILOJS_STATUS_JS_EXCEPTION) return 22;
    if (milojs_exception_length(first) <= 0) return 23;

    if (eval(first, "throw new TypeError('bad')", &value) != MILOJS_STATUS_JS_EXCEPTION) return 24;
    int64_t exception_length = milojs_exception_length(first);
    if (exception_length <= 0 || exception_length >= 64) return 25;
    uint8_t exception[64] = {0};
    if (milojs_exception_copy(first, exception, 63) != exception_length) return 26;
    if (strcmp((const char *)exception, "TypeError: bad") != 0) return 27;

    if (milojs_context_free(first) != MILOJS_STATUS_OK) return 28;
    if (milojs_context_free(first) != MILOJS_STATUS_INVALID_CONTEXT) return 29;
    if (milojs_context_new(&second) != MILOJS_STATUS_OK || second == first) return 30;
    if (milojs_context_free(second) != MILOJS_STATUS_OK) return 31;
