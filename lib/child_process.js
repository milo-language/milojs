...
function AsyncChildProcess(promise) {
  ...
  promise.then(function (res) {
    ...
    self.emit('exit', r.status, null);
    self.emit('close', r.status, null);
  });
}
...
AsyncChildProcess.prototype.kill = function () { this.killed = true; return true; };
...
function execFile(file, args, options, callback) {
  ...
  var promise = __spawnAsync(String(file), toArgs(args), envPairs(options));
  var child = new AsyncChildProcess(promise);
  if (callback) {
    promise.then(function (res) {
      var r = decodeAsync(res);
