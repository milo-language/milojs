...
function toBytes(input, encoding) {
  ...
  // Buffer is a Uint8Array subclass, so numeric indexing reads real bytes; the
  // hash functions below want a plain array they can push onto.
  if (input instanceof Uint8Array) { var out = []; for (var bi = 0; bi < input.length; bi++) out.push(input[bi]); return out; }
...
// GENERATED per platform (tools/gen-os-constants.mjs). OpenSSL's own constants
// do not vary by platform, but they are harvested with the ones that do so there
// is one table and one gate rather than two.
exports.constants = require('_osconstants').crypto;
