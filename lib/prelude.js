...
// Object.prototype.toString.call(process) is "[object process]" in node, from an
// own, non-enumerable, non-configurable tag.
Object.defineProperty(process, Symbol.toStringTag, {
  value: "process", writable: true, enumerable: false, configurable: false,
});
...
//
// ref/unref are engine-backed through `__timerRef`: an unref'd timer still
// fires while the loop is alive for other work but no longer keeps it alive on
// its own, so `setInterval(...).unref()` lets the process exit as it does in
// node. hasRef reports the engine's answer, not a flag kept only here.
(function () {
  ...
  Timeout.prototype.hasRef = function hasRef() { return !this._destroyed && this._refed; };
  Timeout.prototype.ref = function ref() {
    __timerRef(this._id, true);
    this._refed = true;
    return this;
  };
  Timeout.prototype.unref = function unref() {
    __timerRef(this._id, false);
    this._refed = false;
    return this;
  };
