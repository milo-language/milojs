...
AbortSignal.prototype.addEventListener = function (type, fn, opts) {
  ...
  this._listeners.push({ fn: fn, once: !!(opts && opts.once) });
};
...
AbortSignal.prototype.removeEventListener = function (type, fn) {
  ...
  for (var i = 0; i < this._listeners.length; i++) {
    if (this._listeners[i].fn !== fn) kept.push(this._listeners[i]);
  }
...
AbortSignal.prototype.dispatchEvent = function (ev) {
  __abortSignalFire(this, ev);
  return true;
};

function __abortSignalFire(signal, ev) {
  // Copied before iterating: a listener is allowed to remove another one, and
  // a { once: true } listener removes itself.
  var list = signal._listeners.slice();
  signal._listeners = signal._listeners.filter(function (l) { return !l.once; });
  for (var i = 0; i < list.length; i++) list[i].fn.call(signal, ev);
}
...
function __abortSignalAbort(signal, reason) {
  ...
  signal.reason = reason === undefined
    ...
    : reason;
  __abortSignalFire(signal, { type: 'abort', target: signal, currentTarget: signal });
}
