...
URLSearchParams.prototype.get = function (k) { __spBrand(this); for (var i = 0; i < this._p.length; i++) if (this._p[i][0] === k) return this._p[i][1]; return null; };
URLSearchParams.prototype.getAll = function (k) { __spBrand(this); var o = []; for (var i = 0; i < this._p.length; i++) if (this._p[i][0] === k) o.push(this._p[i][1]); return o; };
URLSearchParams.prototype.has = function (k, v) {
  __spBrand(this);
  if (v === undefined) return this.get(k) !== null;
  for (var i = 0; i < this._p.length; i++) if (this._p[i][0] === k && this._p[i][1] === String(v)) return true;
  return false;
};
URLSearchParams.prototype.set = function (k, v) { __spBrand(this); for (var i = 0; i < this._p.length; i++) if (this._p[i][0] === k) { this._p[i][1] = String(v); return; } this._p.push([k, String(v)]); };
URLSearchParams.prototype.append = function (k, v) { __spBrand(this); this._p.push([String(k), String(v)]); };
...
URLSearchParams.prototype['delete'] = function (k, v) {
  ...
  for (var i = 0; i < this._p.length; i++) {
    ...
    if (this._p[i][0] === k && (v === undefined || this._p[i][1] === String(v))) continue;
    kept.push(this._p[i]);
  }
  this._p = kept;
};
...
(function () {
  ...
  if (typeof origNextTick === "function") {
    process.nextTick = function nextTick(callback) {
      ...
      return origNextTick.apply(process, arguments);
    };
  }
