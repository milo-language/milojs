...
getter(DP, "blank", function () {
  var f = slotOf(this, "Duration");
  for (var k in f) { if (k !== "kind" && f[k] !== 0) return false; }
  return true;
});
...
// The designators are case-insensitive, and only the LAST time unit present may
// carry a fraction — "PT0.5H30M" is not half an hour and thirty minutes, it is
// a syntax error. The fraction is exact: 0.999999999 of an hour is 3599999996400
// nanoseconds, so it is carried as an integer count of nanoseconds and split from
// there rather than multiplied out in floating point.
var DURATION_RE = /^([+-])?P(?:(\d+)Y)?(?:(\d+)M)?(?:(\d+)W)?(?:(\d+)D)?(?:(T)(?:(\d+)(?:[.,](\d{1,9}))?H)?(?:(\d+)(?:[.,](\d{1,9}))?M)?(?:(\d+)(?:[.,](\d{1,9}))?S)?)?$/i;
function durationFromString(str) {
  var m = DURATION_RE.exec(str);
  if (!m) throw new RangeError("invalid duration string: " + str);
  var sign = m[1] === "-" ? -1 : 1;
  var hasDate = m[2] !== undefined || m[3] !== undefined || m[4] !== undefined || m[5] !== undefined;
  var hasTime = m[7] !== undefined || m[9] !== undefined || m[11] !== undefined;
  if (!hasDate && !hasTime) throw new RangeError("invalid duration string: " + str);
  if (m[6] !== undefined && !hasTime) throw new RangeError("invalid duration string: " + str + " (time designator with no time)");
  if (m[8] !== undefined && (m[9] !== undefined || m[11] !== undefined)) {
    throw new RangeError("invalid duration string: " + str + " (only the last unit may have a fraction)");
  }
  if (m[10] !== undefined && m[11] !== undefined) {
    throw new RangeError("invalid duration string: " + str + " (only the last unit may have a fraction)");
  }
  function whole(x) { return x === undefined ? 0 : Number(x); }
  function fracNs(digits, unitNs) {
    if (digits === undefined) return 0;
    return Number((digits + "000000000").slice(0, 9)) * (unitNs / 1e9);
  }
  var hours = whole(m[7]), minutes = whole(m[9]), seconds = whole(m[11]);
  var rest = fracNs(m[8], 3600e9) + fracNs(m[10], 60e9) + fracNs(m[12], 1e9);
  if (m[8] !== undefined) { minutes = Math.floor(rest / 60e9); rest -= minutes * 60e9; }
  if (m[8] !== undefined || m[10] !== undefined) { seconds = Math.floor(rest / 1e9); rest -= seconds * 1e9; }
  var ms = Math.floor(rest / 1e6); rest -= ms * 1e6;
  var us = Math.floor(rest / 1e3); rest -= us * 1e3;
  return new Duration(sign * whole(m[2]), sign * whole(m[3]), sign * whole(m[4]), sign * whole(m[5]),
    sign * hours, sign * minutes, sign * seconds, sign * ms, sign * us, sign * rest);
}
...
function spanToDuration(start, end, largest, smallest, increment, mode) {
  ...
  for (var i = 0; i < order.length; i++) {
    ...
    // Seven days is "7 days" unless weeks are what the caller asked for.
    if (u === "week" && largest !== "week") continue;
...
def(DP, "round", function round(roundTo) {
  ...
  var increment = readRoundingIncrement(opts, smallest, "difference");
...
function toOverflow(options, where) {
  // GetOptionsObject runs here rather than at the entry point: `from` processes
  // its ITEM first, so PlainDate.from("nonsense", null) is the string's
  // RangeError and not the options bag's TypeError.
  return getOption(optionsObject(options, where), "overflow", ["constrain", "reject"], "constrain", where);
}
...
// Two two-digit groups and then the end or an annotation: the month-day form.
var MONTH_DAY_SHAPE = /^(?:--)?\d{2}-?\d{2}(?:\[|$)/;
...
function parseTemporalISO(str, what, opts) {
  ...
  var yearMonthOnly = opts && opts.yearMonth;
  ...
  if (timeOnly) {
    ...
    out.hasDate = false;
  } else if (monthDayOnly && MONTH_DAY_SHAPE.test(s)) {
    // "--05-02" and "05-02" are month-days; "1976-11-18" is a whole date and
    // falls through to the general form below.
    if (s.charAt(0) === "-") i = 2;
    out.year = 1972;
    out.month = digits(2);
    if (s.charAt(i) === "-") i++;
    ...
    if (dashed) i++;
    out.month = digits(2);
    // A year-month string may stop after the month; every other form must go on
    // to a day, and the separators have to be used consistently.
    var hasDay = true;
    if (yearMonthOnly) {
      if (dashed) { hasDay = s.charAt(i) === "-"; if (hasDay) i++; }
      else { hasDay = isDigit(s.charAt(i)); }
    } else if (dashed) {
      if (s.charAt(i) !== "-") isoFail(str, what, "inconsistent date separators");
      i++;
    }
    if (hasDay) out.day = digits(2);
  }
...
function PlainDate(y, m, d) {
  ...
  if (arguments[3] !== undefined) toCalendarIdArg(arguments[3]);
  rejectISODate(y, m, d);
...
// CalendarMergeFields. `with` takes a PARTIAL bag: at least one recognised field
// must be present, a `calendar` or `timeZone` property is a TypeError (they are
// not fields you may change this way), and month and monthCode name the same
// thing, so supplying either one drops BOTH of the receiver's rather than
// leaving a stale monthCode to contradict the new month.
function mergeBagFields(base, o, keys, where) {
  if (!isObjectLike(o)) throw new TypeError(where + ": argument must be an object");
  if ("calendar" in o) throw new TypeError(where + ": calendar is not a field");
  if ("timeZone" in o) throw new TypeError(where + ": timeZone is not a field");
  var raw = readBagFields(o, keys);
  if (!raw.$any) throw new TypeError(where + ": at least one field is required");
  var merged = {};
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    if (raw[k] !== undefined) merged[k] = raw[k];
    else if (k === "month" || k === "monthCode") merged[k] = undefined;
    else merged[k] = base[k];
  }
  if (merged.month === undefined && merged.monthCode === undefined && keys.indexOf("month") >= 0) {
...
def(PDP, "with", function withFields(o) {
  ...
  var f = slotOf(this, "PlainDate");
  var merged = mergeBagFields(f, o, DATE_KEYS, "PlainDate.with");
  var r = isoDateFromFields(merged, toOverflow(arguments[1], 'PlainDate.with'), "PlainDate");
...
def(PlainDate, "from", function from(item) {
  ...
  // The ITEM is processed before the options bag is even type-checked, so
  // PlainDate.from("13-34", null) is the string's RangeError.
  return PlainDateFromValue(item, options, 'PlainDate.from');
});
...
def(PTP, "with", function withFields(o) {
  var f = slotOf(this, "PlainTime");
  var merged = mergeBagFields(f, o, TIME_KEYS, "PlainTime.with");
  var t = isoTimeFromFields(merged, toOverflow(arguments[1], 'PlainTime.with'));
...
function stripAnnotations(str) { var b = str.indexOf("["); return b < 0 ? str : str.slice(0, b); }
var AMBIG_YEAR_MONTH = /^\d{4}-?(\d{2})$/;
var AMBIG_MONTH_DAY = /^(\d{2})-?(\d{2})$/;
function isAmbiguousTimeString(str) {
  var s = stripAnnotations(str);
  var m = AMBIG_YEAR_MONTH.exec(s);
  if (m) { var mo = Number(m[1]); return mo >= 1 && mo <= 12; }
  m = AMBIG_MONTH_DAY.exec(s);
  if (m) {
    var mm = Number(m[1]), dd = Number(m[2]);
    // 1972 because that is the month-day reference year, which makes 02-29 real
    return mm >= 1 && mm <= 12 && dd >= 1 && dd <= daysInMonth(1972, mm);
  }
  return false;
}
function PlainTimeFromValue(item, options, where) {
  ...
  var str = item;
  if (str.charAt(0) !== "T" && str.charAt(0) !== "t" && isAmbiguousTimeString(str)) {
    throw new RangeError("invalid PlainTime string: " + str + " (needs a T prefix to be read as a time)");
  }
...
def(PlainTime, "from", function from(item) {
  ...
  // declared second parameter made it 2.
  var options = arguments[1];
  return PlainTimeFromValue(item, options, 'PlainTime.from');
});
...
function PlainDateTime(y, mo, d, h, mi, s, ms, us, ns) {
  ...
  h = requireInt(h, "hour", 0); mi = requireInt(mi, "minute", 0); s = requireInt(s, "second", 0);
  ms = requireInt(ms, "millisecond", 0); us = requireInt(us, "microsecond", 0); ns = requireInt(ns, "nanosecond", 0);
  if (arguments[9] !== undefined) toCalendarIdArg(arguments[9]);
...
def(PDTP, "with", function withFields(o) {
  // options arrive through `arguments` so this method's .length is unchanged
  var f = slotOf(this, "PlainDateTime");
  var merged = mergeBagFields(f, o, DATETIME_KEYS, "PlainDateTime.with");
  var overflow = toOverflow(arguments[1], 'PlainDateTime.with');
  var r = isoDateFromFields(merged, overflow, "PlainDateTime");
  var t = isoTimeFromFields(merged, overflow);
...
def(PlainDateTime, "from", function from(item) {
  ...
  // declared second parameter made it 2.
  var options = arguments[1];
  return PlainDateTimeFromValue(item, options, 'PlainDateTime.from');
});
...
def(Now, "plainDateTimeISO", function plainDateTimeISO() {
  if (arguments[0] !== undefined) toTimeZoneArg(arguments[0]);
  var ms = Date.now();
...
def(Now, "plainTimeISO", function plainTimeISO() {
  if (arguments[0] !== undefined) toTimeZoneArg(arguments[0]);
  return Now.plainDateTimeISO().toPlainTime();
});
...
function PlainYearMonth(y, m, cal, ref) {
  ...
  if (cal !== undefined) toCalendarIdArg(cal);
...
def(YMP, "with", function withFields(o) {
  // options arrive through `arguments` so this method's .length is unchanged
  var f = slotOf(this, "PlainYearMonth");
  var merged = mergeBagFields(f, o, YEARMONTH_KEYS, "PlainYearMonth.with");
  var r = isoDateFromFields(merged, toOverflow(arguments[1], 'PlainYearMonth.with'), "PlainYearMonth");
...
function PlainYearMonthFromValue(item, options, where) {
  ...
  var p = parseTemporalISO(requireISOString(item, "PlainYearMonth"), "PlainYearMonth", { yearMonth: true });
  toOverflow(options, where);
...
def(PlainYearMonth, "from", function from(item) {
  ...
  // declared second parameter made it 2.
  var options = arguments[1];
  return PlainYearMonthFromValue(item, options, 'PlainYearMonth.from');
});
...
def(MDP, "with", function withFields(o) {
  // options arrive through `arguments` so this method's .length is unchanged
  var f = slotOf(this, "PlainMonthDay");
  var merged = mergeBagFields(f, o, DATE_KEYS, "PlainMonthDay.with");
  var r = isoDateFromFields(merged, toOverflow(arguments[1], 'PlainMonthDay.with'), "PlainMonthDay");
  return new PlainMonthDay(r.month, r.day);
});
...
function PlainMonthDayFromValue(item, options, where) {
  ...
  var p = parseTemporalISO(requireISOString(item, "PlainMonthDay"), "PlainMonthDay", { monthDay: true });
  // The year a string carries is only there to say whether the day exists; the
  // stored reference year is always 1972, which is a leap year so 02-29 keeps.
  rejectISODate(p.year, p.month, p.day);
  toOverflow(options, where);
  return new PlainMonthDay(p.month, p.day);
}
...
def(PlainMonthDay, "from", function from(item) {
  ...
  // declared second parameter made it 2.
  var options = arguments[1];
  return PlainMonthDayFromValue(item, options, 'PlainMonthDay.from');
});
...
function ZonedDateTime(ns, tz) {
  ...
  if (arguments[2] !== undefined) toCalendarIdArg(arguments[2]);
  rejectInstant(ns);
  setSlot(this, "ZonedDateTime", { ns: ns, tz: toTimeZoneArg(tz) });
}
...
def(ZonedDateTime, "from", function from(item) {
  ...
  // declared second parameter made it 2.
  var options = arguments[1];
  var result = ZonedDateTimeFromValue(item, options, 'ZonedDateTime.from');
  // Read even when there is nothing to disambiguate: an invalid value for either
  // is a RangeError, not something to shrug at because no transition was hit.
  var resolvedOptions = optionsObject(options, 'ZonedDateTime.from');
  getOption(resolvedOptions, "disambiguation", ["compatible", "earlier", "later", "reject"], "compatible", 'ZonedDateTime.from');
  getOption(resolvedOptions, "offset", ["prefer", "use", "ignore", "reject"], "reject", 'ZonedDateTime.from');
  return result;
});
...
[[PDP, "PlainDate"], [PDTP, "PlainDateTime"], [YMP, "PlainYearMonth"], [MDP, "PlainMonthDay"], [ZP, "ZonedDateTime"]]
  .forEach(function (pair) {
    getter(pair[0], "calendarId", function () { slotOf(this, pair[1]); return "iso8601"; });
  });
// weekOfYear and yearOfWeek exist on every type that has a date, not only on
// PlainDate; they were defined there alone and were missing here entirely.
[[PDTP, "PlainDateTime"], [ZP, "ZonedDateTime"]].forEach(function (pair) {
  getter(pair[0], "weekOfYear", function () {
    var f = localFieldsOf(slotOf(this, pair[1]));
    var thursday = epochDayFromISO(f.year, f.month, f.day) + (4 - dayOfWeek(f.year, f.month, f.day));
    var t = isoFromEpochDay(thursday);
    return Math.floor((thursday - epochDayFromISO(t.year, 1, 1)) / 7) + 1;
  });
  getter(pair[0], "yearOfWeek", function () {
    var f = localFieldsOf(slotOf(this, pair[1]));
    return isoFromEpochDay(epochDayFromISO(f.year, f.month, f.day) + (4 - dayOfWeek(f.year, f.month, f.day))).year;
  });
});
...
def(ZP, "with", function withFields(o) {
  var f = slotOf(this, "ZonedDateTime");
  var l = zonedLocal(f);
  var merged = mergeBagFields(l, o, DATETIME_KEYS, "ZonedDateTime.with");
  var opts = optionsObject(arguments[1], "ZonedDateTime.with");
  // The three options are read even when nothing needs disambiguating, so an
  // invalid one is rejected rather than quietly ignored.
  getOption(opts, "disambiguation", ["compatible", "earlier", "later", "reject"], "compatible", "ZonedDateTime.with");
  getOption(opts, "offset", ["prefer", "use", "ignore", "reject"], "prefer", "ZonedDateTime.with");
  var overflow = toOverflow(opts, "ZonedDateTime.with");
  var r = isoDateFromFields(merged, overflow, "ZonedDateTime");
  var t = isoTimeFromFields(merged, overflow);
  return zonedFromLocal({ year: r.year, month: r.month, day: r.day, hour: t.hour, minute: t.minute,
...
[Duration, PlainDate, PlainTime, PlainDateTime, PlainYearMonth, PlainMonthDay, Instant, ZonedDateTime]
  .forEach(function (c) {
