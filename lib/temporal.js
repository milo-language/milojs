...
def(DP, "toString", function toString() {
  optionsObject(arguments[0], "Duration.toString");
...
function toDurationUnit(v, what) {
  var u = toStringSpec(v, what);
  if (u.charAt(u.length - 1) === "s" && u !== "s") u = u.slice(0, -1);
  if (allUnitIndex(u) < 0) throw new RangeError(what + " must be a Temporal unit, got " + u);
  return u;
}
...
function parseTemporalISO(str, what, opts) {
  ...
  if (out.calendar !== null && out.calendar.toLowerCase() !== "iso8601") {
...
def(PDP, "toString", function toString() {
  var f = slotOf(this, "PlainDate");
  optionsObject(arguments[0], "PlainDate.toString");
  return plainDateToString(f);
});
...
function PlainDateFromValue(item) {
  if (item !== null && typeof item === "object") {
  ...
  var p = parseTemporalISO(item, "PlainDate");
...
def(PlainDate, "from", function from(item) {
  ...
  var resolvedOptions = optionsObject(options, 'PlainDate.from');
  var result = PlainDateFromValue(item);
  toOverflow(resolvedOptions, 'PlainDate.from');
  return result;
});
...
def(PTP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "PlainTime.toString");
  return plainTimeToString(f);
});
...
function PlainTimeFromValue(item) {
  if (item !== null && typeof item === "object") {
    var s = item[$slot];
  ...
  if (typeof item === "symbol") throw new TypeError("cannot convert a symbol to a PlainTime");
  var str = String(item);
...
def(PlainTime, "from", function from(item) {
  ...
  // without ever touching the bag.
  var resolvedOptions = optionsObject(options, 'PlainTime.from');
  var result = PlainTimeFromValue(item);
  toOverflow(resolvedOptions, 'PlainTime.from');
  return result;
});
...
def(PDTP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "PlainDateTime.toString");
  return plainDateTimeToString(f);
});
...
function PlainDateTimeFromValue(item) {
  if (item !== null && typeof item === "object") {
    var s = item[$slot];
  ...
  var p = parseTemporalISO(item, "PlainDateTime");
...
def(PlainDateTime, "from", function from(item) {
  ...
  // without ever touching the bag.
  var resolvedOptions = optionsObject(options, 'PlainDateTime.from');
  var result = PlainDateTimeFromValue(item);
  toOverflow(resolvedOptions, 'PlainDateTime.from');
  return result;
});
...
def(IP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "Instant.toString");
  return instantToString(f.ns);
});
...
def(YMP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "PlainYearMonth.toString");
...
def(PlainYearMonth, "from", function from(item) {
  ...
  // without ever touching the bag.
  var resolvedOptions = optionsObject(options, 'PlainYearMonth.from');
  var result = PlainYearMonthFromValue(item);
  toOverflow(resolvedOptions, 'PlainYearMonth.from');
  return result;
});
...
def(MDP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "PlainMonthDay.toString");
...
var MD_RE = /^(?:--)?(\d{2})-(\d{2})$/;
function PlainMonthDayFromValue(item) {
  if (item !== null && typeof item === "object") {
    var s = item[$slot];
...
def(PlainMonthDay, "from", function from(item) {
  ...
  // without ever touching the bag.
  var resolvedOptions = optionsObject(options, 'PlainMonthDay.from');
  var result = PlainMonthDayFromValue(item);
  toOverflow(resolvedOptions, 'PlainMonthDay.from');
  return result;
});
...
def(ZP, "toString", function toString() {
  ...
  optionsObject(arguments[0], "ZonedDateTime.toString");
  return zonedToString(f);
});
...
function ZonedDateTimeFromValue(item) {
  if (item !== null && typeof item === "object") {
    var s = item[$slot];
...
def(ZonedDateTime, "from", function from(item) {
  ...
  var result = ZonedDateTimeFromValue(item);
  toOverflow(resolvedOptions, 'ZonedDateTime.from');
  return result;
});
...
def(PDTP, "round", function round(options) {
  var f = slotOf(this, "PlainDateTime");
  var total = roundDateTimeFields(f, options, "round");
...
function diffDateTimeFields(a, b, options, where, defaultLargest, isSince) {
  var opts = optionsObject(options, where);
  var smallest = readAnyUnitOption(opts, "smallestUnit", "nanosecond");
  // largestUnit defaults to "auto", which is not the fixed default but the LARGER
  // of that default and smallestUnit — `since(other, {smallestUnit: "year"})` is
  // legal and asks for years, where a fixed default made it "smaller than
  // largestUnit" and threw.
  var largest = readAnyUnitOption(opts, "largestUnit", undefined);
  if (largest === undefined) {
    largest = allUnitIndex(smallest) < allUnitIndex(defaultLargest) ? smallest : defaultLargest;
  }
  var inc = readRoundingIncrement(opts);
  var mode = readRoundingMode(opts);
  if (isSince) mode = negateRoundingMode(mode);
  if (allUnitIndex(smallest) < allUnitIndex(largest)) {
    throw new RangeError("smallestUnit " + smallest + " is larger than largestUnit " + largest);
  }
  if (!isDateUnit(largest)) {
    var d = nsToDuration(dateTimeTotalNs(b) - dateTimeTotalNs(a), largest, smallest, inc, mode);
    return isSince ? d.negated() : d;
  }
  if (isDateUnit(smallest) && smallest !== "day") {
    throw new RangeError("smallestUnit " + smallest + " is not supported for a date-time difference");
  }
  ...
  var dateDur = plainDateDifference({ year: a.year, month: a.month, day: a.day }, endDate,
    { largestUnit: largest }, where);
  var df = durationFields(dateDur);
  // the day is the smallest unit being reported, so the leftover time rounds into it
  var timeDur;
  if (smallest === "day") {
    var extraDays = applyRounding(timeDiff, 86400e9 * inc, mode) / 86400e9;
    df.days += extraDays;
...
function roundDateTimeFields(f, options, where) {
  ...
  var w = ALL_UNITS[allUnitIndex(smallest)][1] * readRoundingIncrement(opts);
  return applyRounding(dateTimeTotalNs(f), w, readRoundingMode(opts));
}
...
def(ZP, "round", function round(options) {
  var f = slotOf(this, "ZonedDateTime");
  var total = roundDateTimeFields(zonedLocal(f), options, "round");
  var days = Math.floor(total / 86400e9);
  var d = isoFromEpochDay(days);
  var t = nsToTime(total - days * 86400e9);
  return zonedFromLocal({ year: d.year, month: d.month, day: d.day, hour: t.hour, minute: t.minute,
...
function toTimeZoneArg(tz) {
  ...
  if (typeof tz === "symbol") throw new TypeError("time zone must be a string");
  return canonicalZone(String(tz));
}
