...
function readUnitValued(opts, key, allowAuto) {
  ...
  var v = opts[key];
  if (v === undefined) return undefined;
  if (allowAuto && typeof v !== "symbol" && String(v) === "auto") return undefined;
  return toDurationUnit(v, key);
}
var UNIT_GROUPS = {
...
function PlainDateFromValue(item) {
  ...
  requireISOString(item, "PlainDate");
...
function PlainDateTimeFromValue(item) {
  ...
  requireISOString(item, "PlainDateTime");
...
function PlainYearMonthFromValue(item) {
  ...
  requireISOString(item, "PlainYearMonth");
  var str = item;
...
function PlainMonthDayFromValue(item) {
  ...
  requireISOString(item, "PlainMonthDay");
  var str = item;
...
function diffTimeLike(aNs, bNs, options, reverse) {
  ...
  var inc = readRoundingIncrement(options, smallest), mode = readRoundingMode(options);
...
def(PTP, "round", function round(options) {
  ...
  var t = nsToTime(applyRounding(timeToNs(slotOf(this, "PlainTime")),
    TIME_UNITS[unitIndex(smallest)][1] * readRoundingIncrement(opts, smallest), readRoundingMode(opts)));
  return new PlainTime(t.hour, t.minute, t.second, t.millisecond, t.microsecond, t.nanosecond);
});
...
def(IP, "round", function round(options) {
  ...
  var smallest = readUnitOption(opts, "smallestUnit", undefined);
  if (smallest === undefined) throw new RangeError("round needs a smallestUnit");
  var w = TIME_UNITS[unitIndex(smallest)][1] * readRoundingIncrement(opts, smallest, "instant");
