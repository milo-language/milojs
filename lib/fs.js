...
function checkOptions(options, name) {
  ...
  if (typeof options === "string") {
    // node reports the ENCODING, whichever way it was passed: the option name is
    // always 'encoding' and the reason is "is invalid encoding".
    if (!_isEncoding(options)) {
      throw _err.ERR_INVALID_ARG_VALUE("encoding", options, "is invalid encoding");
    }
    return;
  }
  ...
  if (options.encoding !== undefined && options.encoding !== null &&
    ...
    throw _err.ERR_INVALID_ARG_VALUE("encoding", options.encoding, "is invalid encoding");
  }
}
...
// node exports the Stats CONSTRUCTOR, and its tests reach for
// `fs.Stats.prototype` and `stats instanceof fs.Stats`. A plain object literal
// answers neither, so four fs tests died reading `prototype` of undefined.
function Stats() {}
Stats.prototype.isFile = function () { return this._isFile; };
Stats.prototype.isDirectory = function () { return this._isDir; };
Stats.prototype.isSymbolicLink = function () { return this._isLink; };
...
// A directory entry with its type, which is what readdir({withFileTypes:true})
// answers. `parentPath` is the current spelling; `path` is the deprecated alias
// node still carries.
...
function makeStats(p, useLstat) {
  ...
  var st = new Stats();
  st.size = size;
  st.mtimeMs = now; st.ctimeMs = now; st.atimeMs = now; st.birthtimeMs = now;
  st.mtime = new Date(now); st.ctime = new Date(now);
  st.atime = new Date(now); st.birthtime = new Date(now);
  st.mode = info ? info.mode : 33188;
  st.ino = 0; st.dev = 0; st.nlink = 1; st.uid = 0; st.gid = 0;
  st.blksize = 4096; st.blocks = Math.ceil(size / 512);
  st._isFile = info ? info.isFile : !isDir;
  st._isDir = isDir;
  st._isLink = isLink;
  return st;
}
...
exports.readdirSync = function (p, opts) {
  ...
  if (opts && typeof opts === 'object' && opts.withFileTypes) {
    ...
    for (var i = 0; i < names.length; i++) {
      // The type comes from a stat of the entry: __readdirSync answers names
      // only, so there is no d_type to read here.
      var full = dir + (dir.charAt(dir.length - 1) === '/' ? '' : '/') + names[i];
      var info = __statSync(full, true);
      var type = 1;
      if (info && info.isSymlink) type = 3;
      else if (info ? info.isDir : __isDir(full)) type = 2;
      out.push(new Dirent(names[i], type, dir));
    }
    return out;
  }
  return names;
...
exports.mkdirSync = function (p, opts) {
  _err.validatePath(p, "path");
  // The mode is validated even though this binding cannot apply it: node's
  // tests pass a bad one and expect the rejection, and a silently accepted mode
  // is how a caller learns nothing until the permissions are wrong.
  if (opts && typeof opts === 'object' && opts.mode !== undefined && opts.mode !== null) {
    var m = opts.mode;
    if (typeof m === 'string') {
      var parsedMode = parseInt(m, 8);
      if (isNaN(parsedMode)) {
        throw _err.ERR_INVALID_ARG_VALUE_PROP('options.mode', m,
          'must be a 32-bit unsigned integer or an octal string');
      }
    } else if (typeof m !== 'number' || !isFinite(m) || m < 0 || m > 4294967295 || Math.floor(m) !== m) {
      throw _err.ERR_INVALID_ARG_VALUE_PROP('options.mode', m,
        'must be a 32-bit unsigned integer or an octal string');
    }
  }
  p = String(p);
  if (opts && opts.recursive) return mkdirp(p, p);
  var r = __mkdirSync(p);
  if (r !== true) throw mkdirError(-r, p);
};

// libuv's text for the codes mkdir can fail with; node's message is
// "<code>: <text>, mkdir '<path>'". The native hands back the errno, so the
// name comes from the platform table rather than a guess from existsSync.
var UV_TEXT = {
  ...
  ENAMETOOLONG: "name too long", ENOSPC: "no space left on device",
  ELOOP: "too many symbolic links encountered", EINVAL: "invalid argument",
};
function errnoName(errno) {
  var table = require('_osconstants').errno;
  for (var k in table) if (table[k] === errno) return k;
  return "EIO";
}
function mkdirError(errno, p) {
  var code = errnoName(errno);
  var e = new Error(code + ": " + (UV_TEXT[code] || "i/o error") + ", mkdir '" + p + "'");
  e.errno = -errno; e.code = code; e.syscall = "mkdir"; e.path = p;
  return e;
}
function parentDir(p) {
  var end = p.length;
  while (end > 1 && p[end - 1] === '/') end--;
  var cut = p.lastIndexOf('/', end - 1);
  if (cut < 0) return '.';
  if (cut === 0) return '/';
  return p.slice(0, cut);
}
// mkdir -p the way libuv's mkdirp does it: try the leaf, recurse on ENOENT,
// and treat EEXIST as fine only when the thing there is a directory. Returns
// the first directory created, or undefined when nothing was. Every error
// names the path the caller asked for, as node's does.
function mkdirp(p, requested) {
  var r = __mkdirSync(p);
  if (r === true) return p;
  var code = errnoName(-r);
  if (code === "ENOENT") {
    var parent = parentDir(p);
    if (parent === p) throw mkdirError(-r, requested);
    var first = mkdirp(parent, requested);
    r = __mkdirSync(p);
    if (r === true) return first === undefined ? p : first;
    code = errnoName(-r);
    if (code === "EEXIST" && __isDir(p)) return first;
    throw mkdirError(-r, requested);
  }
  if (code === "EEXIST" && __isDir(p)) return undefined;
  throw mkdirError(-r, requested);
}
...
// GENERATED per platform (tools/gen-os-constants.mjs): the O_* flags this file
// used to carry were linux's (O_CREAT 64) while os.js carried darwin's errno,
// in the same runtime. O_SYMLINK is darwin-only and its ABSENCE is what node
// gates fs.lchmod/lchmodSync on, so the platform split has to be in the table
// rather than patched on after it.
exports.constants = require('_osconstants').fs;
...
exports.existsSync = existsSync;
// The callback is required and must be a function: fs.exists(path, 'nope') is
// ERR_INVALID_ARG_TYPE, not a call that quietly does nothing.
exports.exists = function (p, cb) { _err.validateCallback(cb); cb(existsSync(p)); };
...
function checkChmod(p, mode) {
  ...
  if (typeof mode === "string") {
    // The WHOLE string has to be octal. parseInt stops at the first character it
    // cannot use, so "123x" parsed as 0o123 and the file was then chmod'd to a
    // mode nobody asked for — a validation bug that WRITES. node rejects it.
    if (!/^[0-7]+$/.test(mode)) {
      throw _err.ERR_INVALID_ARG_VALUE("mode", mode, "must be a 32-bit unsigned integer or an octal string");
    }
...
// The native answers a bare false for every failure, so the errno has to be
// reconstructed here: a missing path is ENOENT, and anything else the kernel
// refused on a path that IS there is reported as EPERM, which is what chmod
// fails with when the caller does not own the file. Returning silently instead
// meant fs.chmodSync('/no/such/path') looked like it had worked.
function doChmod(p, mode, syscall) {
  checkChmod(p, mode);
  if (__chmodSync(String(p), Number(mode))) return;
  var missing = !existsSync(String(p));
  var e = new Error((missing ? "ENOENT: no such file or directory, " : "EPERM: operation not permitted, ") +
    syscall + " '" + String(p) + "'");
  e.code = missing ? "ENOENT" : "EPERM";
  e.errno = missing ? -2 : -1;
  e.syscall = syscall;
  e.path = String(p);
  throw e;
}
exports.chmodSync = function (p, mode) { doChmod(p, mode, "chmod"); };
exports.chmod = function (p, mode, cb) {
  checkChmod(p, mode); _err.validateCallback(cb);
  try { doChmod(p, mode, "chmod"); } catch (e) { cb(e); return; }
  cb(null);
};
exports.chownSync = function (p, uid, gid) { checkChown(p, uid, gid); };
// lchmod/lchown act on the LINK rather than its target. There is no binding for
// either, so these validate exactly as their non-l siblings do and then do what
// chmod/chown do here — the argument checking is what node's tests for them
// assert, and silently missing functions failed before reaching it.
// ...except that lchmod is DARWIN-ONLY. node defines it as
// `constants.O_SYMLINK !== undefined ? lchmod : undefined`, so on linux the
// property is absent entirely. Defining it unconditionally gave milojs an
// export node does not have on the platform CI runs, which no fixture could
// pin: one .expected cannot hold both platforms' answers.
if (exports.constants.O_SYMLINK !== undefined) {
  exports.lchmodSync = function (p, mode) { doChmod(p, mode, "lchmod"); };
  exports.lchmod = function (p, mode, cb) {
    checkChmod(p, mode); _err.validateCallback(cb);
    try { doChmod(p, mode, "lchmod"); } catch (e) { cb(e); return; }
    cb(null);
  };
}
exports.lchownSync = function (p, uid, gid) { checkChown(p, uid, gid); };
exports.lchown = function (p, uid, gid, cb) { checkChown(p, uid, gid); _err.validateCallback(cb); cb(null); };
// promises.lchmod is the one that EXISTS on both platforms: node keeps the
// property and rejects with ERR_METHOD_NOT_IMPLEMENTED where the callback form
// is simply absent.
exports.promises.lchmod = function (p, mode) {
  return new Promise(function (res, rej) {
    if (exports.constants.O_SYMLINK === undefined) {
      var e = new Error('The lchmod() method is not implemented');
      e.code = 'ERR_METHOD_NOT_IMPLEMENTED';
      rej(e);
      return;
    }
    try { exports.lchmodSync(p, mode); res(); } catch (e2) { rej(e2); }
  });
};
exports.promises.lchown = function (p, uid, gid) {
  return new Promise(function (res, rej) { try { exports.lchownSync(p, uid, gid); res(); } catch (e) { rej(e); } });
};
...
// --- argument validation -------------------------------------------------
//
// Applied as one wrapping pass rather than as sixty hand-written checks inside
// sixty function bodies. Node's fs tests overwhelmingly assert on the error's
// `code`, and a third of the fs area's failures were this one thing: a bad path
// or a bad callback went through, and either succeeded or threw a plain
// assertion instead of the coded TypeError.
//
// The table is the specification. A function absent from it is validated for
// nothing, which is the safe default when a signature is unusual.
var _v = require("_errors");

// Positional arguments that must be a path. Note what is NOT here: existsSync
// and exists never throw in node, they answer false, so validating their path
// would turn a documented answer into an exception.
var PATH_ARGS = {
  open: [0], openSync: [0],
  ...
  chmod: [0], chmodSync: [0], lchmod: [0], lchmodSync: [0],
...
// Positional arguments that must be a file descriptor.
var FD_ARGS = {
  close: [0], closeSync: [0],
  read: [0], readSync: [0], write: [0], writeSync: [0],
  fsync: [0], fsyncSync: [0], fdatasync: [0], fdatasyncSync: [0],
  ftruncate: [0], ftruncateSync: [0],
  fchmod: [0], fchmodSync: [0], fchown: [0], fchownSync: [0],
  fstat: [0], fstatSync: [0],
  futimes: [0], futimesSync: [0],
};

// readFile/writeFile/appendFile accept a NUMBER as their first argument and
// treat it as an already-open file descriptor, so a number there is not a bad
// path. Every other path-taking function rejects it.
var FD_AS_PATH = { readFile: 1, readFileSync: 1, writeFile: 1, writeFileSync: 1, appendFile: 1, appendFileSync: 1 };

// Where each async function's callback sits. node resolves it from the
// SIGNATURE, not from "the last argument": fs.stat("/tmp", 5) treats 5 as the
// options bag and reports the callback as undefined, while fs.unlink(p, null)
// — which has no options parameter — reports null. A "last argument" rule gets
// the first of those wrong.
var CB_SLOT = {
  access: [2, true], appendFile: [3, true], chmod: [2, false], chown: [3, false],
  close: [1, false], copyFile: [3, true], fchmod: [2, false], fchown: [3, false],
  fdatasync: [1, false], fstat: [2, true], fsync: [1, false], ftruncate: [2, true],
  futimes: [3, false], lchmod: [2, false], lchown: [3, false], link: [2, false],
  lstat: [2, true], lutimes: [3, false], mkdir: [2, true], mkdtemp: [2, true],
  readdir: [2, true], readFile: [2, true], readlink: [2, true], realpath: [2, true],
  rename: [2, false], rm: [2, true], rmdir: [2, true], stat: [2, true],
  symlink: [3, true], truncate: [2, true], unlink: [1, false], utimes: [3, false],
  writeFile: [3, true],
};

function resolveCallback(name, args) {
  var slot = CB_SLOT[name];
  if (!slot) {
    // open/read/write have variadic shapes; the last argument is the best
    // available answer and matches node for every call that passes one.
    return args.length > 0 ? args[args.length - 1] : undefined;
  }
  var cb = args[slot[0]];
  // The optional options bag collapses: a function in its slot IS the callback.
  if (slot[1] && typeof args[slot[0] - 1] === 'function') cb = args[slot[0] - 1];
  return cb;
}

function argName(paths, index) {
  if (paths.length === 1) return 'path';
  return index === 0 ? 'oldPath' : 'newPath';
}

// A NUL byte terminates the string at the syscall boundary, so a path
// containing one addresses a different file than the one written. Node rejects
// it as ERR_INVALID_ARG_VALUE rather than letting the truncation happen.
function checkNulBytes(name, value) {
  var s = null;
  if (typeof value === 'string') s = value;
  else if (value instanceof Uint8Array) s = String.fromCharCode.apply(null, value);
  else if (value && typeof value.pathname === 'string') s = value.pathname;
  if (s !== null && s.indexOf('\u0000') !== -1) {
    throw _v.ERR_INVALID_ARG_VALUE(name, value,
      'must be a string, Uint8Array, or URL without null bytes');
  }
}

function validateFd(value, name) {
  if (typeof value !== 'number') throw _v.ERR_INVALID_ARG_TYPE(name, 'of type number', value);
  // Node reports the integer requirement and the range as two different
  // messages, and checks integrality first.
  if (!Number.isInteger(value)) throw _v.ERR_OUT_OF_RANGE(name, 'an integer', value);
  if (value < 0 || value > 2147483647) {
    throw _v.ERR_OUT_OF_RANGE(name, '>= 0 && <= 2147483647', value);
  }
}

function wrapFsFunction(name, fn) {
  var paths = PATH_ARGS[name];
  var fds = FD_ARGS[name];
  if (!paths && !fds) return fn;
  // The async half of every pair takes a trailing callback, and node throws
  // when it is absent or is not a function rather than proceeding silently.
  var wantsCallback = !/Sync$/.test(name);
  var wrapped = function () {
    var i;
    if (paths) {
      for (i = 0; i < paths.length; i++) {
        var nm = argName(paths, paths[i]);
        var pv = arguments[paths[i]];
        if (FD_AS_PATH[name] && paths[i] === 0 && typeof pv === 'number') continue;
        _v.validatePath(pv, nm);
        checkNulBytes(nm, pv);
      }
    }
    if (fds) {
      for (i = 0; i < fds.length; i++) validateFd(arguments[fds[i]], 'fd');
    }
    if (wantsCallback) {
      var cb = resolveCallback(name, arguments);
      if (typeof cb !== 'function') throw _v.ERR_INVALID_ARG_TYPE('cb', 'of type function', cb);
    }
    return fn.apply(this, arguments);
  };
  // name and length are observable, and node's own tests read them off fs
  // functions.
  try {
    Object.defineProperty(wrapped, 'name', { value: name, configurable: true });
    Object.defineProperty(wrapped, 'length', { value: fn.length, configurable: true });
  } catch (e) { /* a host that will not redefine these is not worth failing over */ }
  return wrapped;
}

// --- promises.open / FileHandle ---------------------------------------------
//
// The promise API's handle over an fd. Four fs tests call fsPromises.open
// directly and every one of them died on "open is not a function"; more reach
// it through fs.promises.readFile of an already-open handle.
...
FileHandle.prototype.close = function () {
  var self = this;
  return new Promise(function (res, rej) {
    if (self._closed) return res();
    try { exports.closeSync(self.fd); self._closed = true; res(); } catch (e) { rej(e); }
  });
};
FileHandle.prototype.read = function (bufferOrOpts, offset, length, position) {
  var self = this;
  var buffer = bufferOrOpts;
  if (bufferOrOpts && !(bufferOrOpts instanceof Uint8Array)) {
    var o = bufferOrOpts || {};
    buffer = o.buffer || require('buffer').Buffer.alloc(16384);
  ...
  return new Promise(function (res, rej) {
    try {
      var n = exports.readSync(self.fd, buffer, offset, length, position);
      res({ bytesRead: n, buffer: buffer });
    } catch (e) { rej(e); }
  });
};
FileHandle.prototype.write = function (data, offsetOrPosition, lengthOrEncoding, position) {
  var self = this;
  return new Promise(function (res, rej) {
    try {
      var n = exports.writeSync(self.fd, data, offsetOrPosition, lengthOrEncoding, position);
      res({ bytesWritten: n, buffer: data });
    } catch (e) { rej(e); }
  });
};
FileHandle.prototype.writeFile = function (data, opts) {
  var self = this;
  return new Promise(function (res, rej) {
    try { exports.writeSync(self.fd, data, null, opts && opts.encoding); res(); } catch (e) { rej(e); }
  });
};
FileHandle.prototype.readFile = function (opts) {
  var self = this;
  return new Promise(function (res, rej) {
    try { res(exports.readFileSync(self._path, opts)); } catch (e) { rej(e); }
  });
};
FileHandle.prototype.appendFile = function (data, opts) {
  return this.writeFile(data, opts);
};
FileHandle.prototype.stat = function () {
  var self = this;
  return new Promise(function (res, rej) {
    try { res(statSync(self._path)); } catch (e) { rej(e); }
  });
};
FileHandle.prototype.truncate = function (len) {
  var self = this;
  return new Promise(function (res, rej) {
    try { exports.ftruncateSync(self.fd, len); res(); } catch (e) { rej(e); }
  });
};
FileHandle.prototype.chmod = function (mode) {
  var self = this;
  return new Promise(function (res, rej) {
    try { exports.chmodSync(self._path, mode); res(); } catch (e) { rej(e); }
  });
};
// sync(2)/fdatasync(2) have no binding here, and a handle that cannot flush is
// better than one that pretends to fail: the write already went through
// writeSync, which is not buffered on this side.
...
exports.promises.open = function (p, flags, mode) {
  return new Promise(function (res, rej) {
    try {
      var fd = exports.openSync(p, flags === undefined ? 'r' : flags, mode);
      res(new FileHandle(fd, String(p)));
    } catch (e) { rej(e); }
  });
};
...
// --- writev / readv ----------------------------------------------------------
//
// Scatter/gather over the same fd, which node's streams use to flush a corked
// write in one call. Implemented as a loop over the buffers: the syscall is not
// bound here, and the OBSERVABLE result (bytes written, in order) is the same.
exports.writevSync = function (fd, buffers, position) {
  _err.validateInteger(fd, 'fd', 0);
  var total = 0;
  for (var i = 0; i < buffers.length; i++) {
    var b = buffers[i];
    var pos = position === undefined || position === null ? null : position + total;
    total += exports.writeSync(fd, b, 0, b.length, pos);
  }
  return total;
};
exports.writev = function (fd, buffers, positionOrCb, maybeCb) {
  var cb = typeof positionOrCb === 'function' ? positionOrCb : maybeCb;
  var position = typeof positionOrCb === 'function' ? null : positionOrCb;
  _err.validateCallback(cb);
  try { var n = exports.writevSync(fd, buffers, position); cb(null, n, buffers); }
  catch (e) { cb(e); }
};
exports.readvSync = function (fd, buffers, position) {
  ...
  for (var i = 0; i < buffers.length; i++) {
    var b = buffers[i];
    var pos = position === undefined || position === null ? null : position + total;
    var n = exports.readSync(fd, b, 0, b.length, pos);
    total += n;
    if (n < b.length) break;
  }
  return total;
};
exports.readv = function (fd, buffers, positionOrCb, maybeCb) {
  var cb = typeof positionOrCb === 'function' ? positionOrCb : maybeCb;
  var position = typeof positionOrCb === 'function' ? null : positionOrCb;
  _err.validateCallback(cb);
  try { var n = exports.readvSync(fd, buffers, position); cb(null, n, buffers); }
  catch (e) { cb(e); }
};
...
exports.promises.rmdir = function (p, opts) {
  return new Promise(function (res, rej) {
    try { exports.rmdirSync(p, opts); res(); } catch (e) { rej(e); }
  });
};
exports.promises.chown = function (p, uid, gid) {
  return new Promise(function (res, rej) {
    try { exports.chownSync(p, uid, gid); res(); } catch (e) { rej(e); }
  });
};
exports.promises.utimes = function () { return Promise.resolve(); };

(function applyValidation() {
  var names = Object.keys(exports);
  for (var i = 0; i < names.length; i++) {
    var n = names[i];
    if (typeof exports[n] !== 'function') continue;
    var w = wrapFsFunction(n, exports[n]);
    if (w !== exports[n]) {
      // realpath carries a .native sibling; wrapping must not drop it.
      var extras = Object.keys(exports[n]);
      for (var j = 0; j < extras.length; j++) w[extras[j]] = exports[n][extras[j]];
      exports[n] = w;
    }
  }
})();
