...
// --- OutgoingMessage ---------------------------------------------------------
//
// node's base class for ServerResponse and ClientRequest. It exists here mostly
// for its VALIDATION: the header setters are where node rejects a bad token, a
// value it cannot render, and a write after the headers went out, each with a
// code its tests assert directly. Twelve http tests construct one by hand, and
// they all died on `new OutgoingMessage()` not being a constructor.

// RFC 7230 token characters. A header name outside this set cannot be written
// without corrupting the message framing, so node refuses rather than escaping.
var TOKEN_RE = /^[\^_`a-zA-Z\-0-9!#$%&'*+.|~]+$/;
// A header VALUE may not carry a control character or a newline: a newline ends
// the header and everything after it is read as the next one (response
// splitting). node also rejects anything above latin1, which it cannot encode.
var INVALID_VALUE_RE = /[^\t\x20-\x7e\x80-\xff]/;
...
function validateHeaderValue(name, value, kind) {
  if (value === undefined) {
    throw _httpErr.codedError(TypeError, 'ERR_HTTP_INVALID_HEADER_VALUE',
      'Invalid value "' + value + '" for header "' + name + '"');
  }
  if (INVALID_VALUE_RE.test(String(value))) {
    throw _httpErr.codedError(TypeError, 'ERR_INVALID_CHAR',
      'Invalid character in ' + (kind || 'header') + ' content ["' + name + '"]');
  }
}

function checkHeadersSent(msg) {
  if (msg._header) {
    throw _httpErr.codedError(Error, 'ERR_HTTP_HEADERS_SENT',
...
function OutgoingMessage() {
  ...
  this._trailer = '';
  this._headers = {};
  // The raw spelling each header was set with, so getHeaderNames and the header
  // block can answer in the caller's case rather than the lowercased key.
  this._headerNames = {};
  this._header = null;
...
OutgoingMessage.prototype.getRawHeaderNames = function () {
  var keys = Object.keys(this._headers);
  var out = [];
  for (var i = 0; i < keys.length; i++) out.push(this._headerNames[keys[i]] || keys[i]);
  return out;
};
...
OutgoingMessage.prototype._renderHeaders = function () {
  ...
  for (var i = 0; i < keys.length; i++) {
    out[this._headerNames[keys[i]] || keys[i]] = this._headers[keys[i]];
  }
  return out;
};
// The subclass decides how a header block is produced; the base class has no
// way to, and node reports that rather than silently doing nothing.
OutgoingMessage.prototype._implicitHeader = function () {
  throw _httpErr.codedError(Error, 'ERR_METHOD_NOT_IMPLEMENTED',
    'The _implicitHeader() method is not implemented');
};
function validateChunk(chunk) {
  if (chunk === null) {
    throw _httpErr.codedError(TypeError, 'ERR_STREAM_NULL_VALUES',
      'May not write null values to stream');
  }
  if (typeof chunk !== 'string' && !(chunk instanceof Uint8Array)) {
    throw _httpErr.ERR_INVALID_ARG_TYPE('chunk',
...
OutgoingMessage.prototype.addTrailers = function (headers) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (Array.isArray(headers) && headers[i] && headers[i].length === 2) {
      field = headers[i][0];
      value = headers[i][1];
    }
    validateHeaderName(field, 'Trailer name');
    validateHeaderValue(field, value, 'trailer');
    this._trailer += field + ': ' + value + '\r\n';
  }
};
OutgoingMessage.prototype.cork = function () { this._corked++; };
OutgoingMessage.prototype.uncork = function () { if (this._corked > 0) this._corked--; };
OutgoingMessage.prototype.destroy = function (error) {
  if (this.destroyed) return this;
  this.destroyed = true;
  if (this.socket && typeof this.socket.destroy === 'function') this.socket.destroy(error);
  return this;
};
...
// A response is attached to its socket AFTER construction (an upgrade hands one
// over), and node's own ServerResponse.assignSocket is what test harnesses use
// to drive a response without a server.
...
OutgoingMessage.prototype.pipe = function () {
  throw _httpErr.codedError(Error, 'ERR_STREAM_CANNOT_PIPE', 'Cannot pipe, not readable');
};
...
Object.defineProperty(OutgoingMessage.prototype, 'writableHighWaterMark', {
  get: function () { return this.socket ? this.socket.writableHighWaterMark : 16384; },
  configurable: true,
});
Object.defineProperty(OutgoingMessage.prototype, 'writableLength', {
  get: function () { return this.outputSize + (this.socket ? this.socket.writableLength || 0 : 0); },
  configurable: true,
});
Object.defineProperty(OutgoingMessage.prototype, 'writableCorked', {
  get: function () { return this._corked; }, configurable: true,
});
...
ServerResponse.prototype._sendHeaders = function () {
  ...
  // be revised.
  // Chunked is an HTTP/1.1 framing. An HTTP/1.0 client does not understand it
  // and reads the size lines as body, so for 1.0 the body is delimited by the
  // connection close instead — which `Connection: close` below already
  // arranges. node makes the same distinction.
  if (body['content-length'] === undefined && this._streaming && this._httpVersion !== '1.0') {
...
ServerResponse.prototype.write = function (chunk, encoding, cb) {
  ...
  if (this.headersSent) {
    ...
    // node commits the headers on the FIRST write and frames the rest chunked;
    // it does not wait for end(). Holding the chunk instead bought an exact
    // Content-Length, and cost every streaming response: a handler that wrote a
    // first chunk and kept the connection open sent nothing at all, so the
    // client sat waiting for a head that only end() would produce. That is what
    // made the server-side abort tests hang — the client never got far enough
    // to abort anything.
    this._streaming = true;
    this._sendHeaders();
    var m = chunkByteLength(piece);
    if (this._chunked) {
      if (m > 0) {
        __tcpSend(this._connId, m.toString(16) + '\r\n');
        sendChunk(this._connId, piece);
        __tcpSend(this._connId, '\r\n');
      }
    } else if (piece !== null) {
...
ServerResponse.prototype.end = function (chunk, encoding, cb) {
  ...
  // finished BEFORE the close, so the socket's own 'close' handler can tell an
  // orderly end from a client that vanished.
  this._sent = true;
  this.headersSent = true;
  this.finished = true;
  this.writable = false;
  // Through the Socket when there is one, so it leaves the event loop's pump
  // list; a raw __tcpClose would leave a wrapper pointing at a dead id.
  if (this.socket && typeof this.socket.destroy === 'function') this.socket.destroy();
  else __tcpClose(this._connId);
  this.emit('finish');
  this.emit('close');
  if (cb) cb();
  return this;
...
Server.prototype._serveOnce = function (mayBlock) {
  ...
  // The response's framing depends on what the CLIENT spoke, so the version
  // travels from the parsed request to the response.
  res._httpVersion = parsed.httpVersion;

  // The accepted connection used to be a bare integer, so nothing could observe
  // the client going away: req 'aborted' had nothing to fire from, and every
  // node test that waits for it was killed by the harness. Wrapping it in the
  // same Socket the client side already uses makes the disconnect an event.
  var sock = null;
  try { sock = require('net')._adoptConnection(connId); } catch (e) { sock = null; }
  if (sock) {
    sock.remoteAddress = '127.0.0.1';
    req.socket = sock; req.connection = sock;
    res.socket = sock; res.connection = sock;
    var gone = function () {
      // A response that already finished closed its own socket; that is not an
      // abort, and reporting it as one would fire 'aborted' on every request.
      if (res.finished || req.aborted) return;
      req.aborted = true;
      req.emit('aborted');
      req.emit('close');
      res.emit('close');
...
function sendOverSocket(req, host, port, path) {
  ...
  __armRequestTimeout(req, socket);
  // node emits 'socket' on the request as soon as one is assigned, and tests
  // hang waiting for it.
  setTimeout(function () { req.emit('socket', socket); }, 0);
  ...
  // node emits 'close' on the REQUEST once the exchange is over, not only when
  // it is destroyed. Without it a caller that waits for req 'close' to tear
  // down its server waits forever, and the process never exits: that is a
  // hang reported as a timeout, with nothing in the output to say why.
  var exchangeDone = false;
  var finishExchange = function () {
    if (exchangeDone) return;
    exchangeDone = true;
    setTimeout(function () { if (!req._closeEmitted) { req._closeEmitted = true; req.emit('close'); } }, 0);
  };

  socket.on('data', function (d) {
    // An aborted request delivers nothing further. Before responses streamed
    // this could not arise — the whole body arrived in one piece or not at all
    // — but now that a server can write before it ends, bytes are genuinely in
    // flight when abort() lands, and node drops them.
    ...
    for (;;) {
      ...
      if (state === 'body') {
        ...
        res._push(piece);
        if (remaining === 0) { state = 'done'; res._streamEnd(); finishExchange(); }
        return;
      }
      if (state === 'chunk') {
        var nl = buf.indexOf('\r\n');
        ...
        if (semi >= 0) sizeLine = sizeLine.slice(0, semi);
        var n = parseInt(sizeLine.trim(), 16);
        if (isNaN(n)) { state = 'done'; res._streamEnd(); finishExchange(); return; }
        if (n === 0) { state = 'done'; res._streamEnd(); finishExchange(); return; }
        ...
        // Re-checked per chunk, not once on entry: a 'data' handler is allowed
        // to abort, and node's own test does exactly that from inside the
        // handler, so several more chunks would otherwise be delivered from the
        // same pass of this loop after the request was already aborted.
        if (req.destroyed || req.aborted) return;
        continue;
      }
      return;
...
function ClientRequest(options, cb) {
  ...
  this._chunks = [];
  this.finished = false;
  // `http.request({ timeout })` is the option spelling of req.setTimeout, and
  // node applies it to the socket the same way.
  this._timeoutMs = opts.timeout === undefined ? 0 : Number(opts.timeout) || 0;
  this._timeoutBound = false;
  // Both are read directly by callers deciding whether a request is still live,
  // so they are present from construction rather than appearing on first abort.
  this.aborted = false;
  this.destroyed = false;
...
ClientRequest.prototype = Object.create(OutgoingMessage.prototype);
...
// Arms the socket's idle timer and forwards its 'timeout' to the request. The
// socket may not exist yet — setTimeout is usually called before end() — so the
// pending value is stored and applied when sendOverSocket assigns one.
ClientRequest.prototype.setTimeout = function (msecs, cb) {
  if (typeof cb === 'function') this.once('timeout', cb);
  this._timeoutMs = Number(msecs) || 0;
  if (this.socket) __armRequestTimeout(this, this.socket);
  return this;
};

function __armRequestTimeout(req, socket) {
  if (!req._timeoutMs) return;
  socket.setTimeout(req._timeoutMs);
  if (!req._timeoutBound) {
    req._timeoutBound = true;
    // node reports and lets the caller decide; it does not destroy the request.
    socket.on('timeout', function () { req.emit('timeout'); });
  }
}
...
ClientRequest.prototype.destroy = function (err) {
  ...
  if (this.socket && typeof this.socket.destroy === 'function') this.socket.destroy();
  var self = this;
  // Destroying a request whose response never arrived is a connection reset,
  // and node reports it: "socket hang up", code ECONNRESET. abort() is the one
  // exception — it is a deliberate cancellation, so it emits 'abort' and no
  // error, which is why node's own abort tests assert the error never fires.
  var reset = null;
  if (!err && !this.aborted && this.finished && !this.res) {
    reset = new Error('socket hang up');
    reset.code = 'ECONNRESET';
  }
  setTimeout(function () {
    if (err) self.emit('error', err);
    else if (reset) self.emit('error', reset);
    self.emit('close');
  }, 0);
  return this;
};
ClientRequest.prototype.write = function (chunk, encoding, cb) {
  if (this.destroyed || this.aborted) return false;
...
ClientRequest.prototype.end = function (chunk, encoding, cb) {
  ...
  this.finished = true;
  // `req.abort(); req.end();` is an ordinary shape — node's own tests write it
  // exactly that way. Sending anyway opened a socket to a server the test had
  // already decided must never be contacted, and that socket then held the
  // event loop open forever: the request was aborted, the connection was not.
