...
// ToString of an import() specifier: the template applies the string hint
// (toString before valueOf) and throws for a symbol, as the spec's step does.
function __importSpec(s) { return `${s}`; }

// The success path of import(spec) resolves through this identity, one
// microtask after the load, so it settles after any rejection import()
// produced synchronously, as node's loader does.
function __importSettle(ns) { return ns; }

// The second argument of import(spec, options): node's checks and messages.
// The attributes are read (so a getter on `with` runs, as the spec has it)
// and then dropped, because the loader takes none.
function __importOptions(options) {
  if (options === undefined) return;
  if (options === null || (typeof options !== "object" && typeof options !== "function")) {
    throw new TypeError("The second argument to import() must be an object");
  }
  var attrs = options.with;
...
var Reflect = {
  ...
  deleteProperty: function (target, key) {
    ...
    // __reflectDelete, not `delete`: this file is strict, and in strict code
    // `delete` on a non-configurable property is a TypeError. Reflect's whole
    // contract is to answer false there instead.
    return __reflectDelete(target, __reflectKey(key));
  },
...
// A DEEP copy that preserves the shape of the reference graph: a cycle stays a
// cycle and two properties pointing at one object still point at one object after
// the clone. That is what a memo of original -> clone buys, and it is the whole
// reason this cannot be `JSON.parse(JSON.stringify(x))`.
//
// The prototype is deliberately DROPPED (a cloned class instance is a plain
// object), getters are evaluated rather than copied as accessors, and anything
// with no serialised form -- a function or a symbol, at any depth -- is a
// DataCloneError rather than a silent omission.
globalThis.structuredClone = function structuredClone(value, options) {
  ...
  function cloneVal(v) {
    ...
    if (t === "function" || t === "symbol") fail(v);

    // The memo is consulted BEFORE any construction and written immediately
    // after, so a container that contains itself finds the half-built clone
    // rather than recursing.
    if (memo.has(v)) return memo.get(v);

    if (v instanceof Date) { var d = new Date(v.getTime()); memo.set(v, d); return d; }
    if (v instanceof RegExp) { var r = new RegExp(v.source, v.flags); memo.set(v, r); return r; }
    if (v instanceof ArrayBuffer) { var b = v.slice(0); memo.set(v, b); return b; }
    if (typeof v.byteLength === "number" && typeof v.BYTES_PER_ELEMENT === "number") {
      var ta = new v.constructor(v);
      memo.set(v, ta);
      return ta;
    }
    if (v instanceof Map) {
      var m = new Map();
      memo.set(v, m);
      v.forEach(function (val, k) { m.set(cloneVal(k), cloneVal(val)); });
      return m;
    }
    if (v instanceof Set) {
      var st2 = new Set();
      memo.set(v, st2);
      v.forEach(function (x) { st2.add(cloneVal(x)); });
      return st2;
    }
    if (v instanceof Error) {
      var e = new v.constructor(v.message);
      memo.set(v, e);
    ...
    for (var j = 0; j < ks.length; j++) o[ks[j]] = cloneVal(v[ks[j]]);
    return o;
  }
  var cloned = cloneVal(value);
  // The transfer list DETACHES each buffer it names: its memory moves to the
  // clone rather than being copied, and reads through any view on it answer
  // undefined afterwards. Ignoring the option silently left the source usable,
  // so code that transfers a buffer and then checks it was neutered saw the
  // opposite of what every other engine reports.
  if (options != null && typeof options === "object" && options.transfer != null) {
    ...
    for (var ti = 0; ti < list.length; ti++) {
      ...
      if (item instanceof ArrayBuffer) {
        if (item.detached) {
          throw new DOMException("ArrayBuffer is already detached.", "DataCloneError");
        }
        item.transfer();
      } else {
...
// --- RegExp protocol, in the spec's direction --------------------------------
// String.prototype.match/replace/search/split/matchAll and RegExp.prototype's
// test and symbol methods run natively while RegExp.prototype is untouched and
// the regex has no own `exec` (regexIsPlain in eval.milo). Every other receiver
// lands here: a patched exec, a subclass, a plain object with an exec method.
// Each function is the spec algorithm, and each reaches the regex only through
// RegExpExec, which calls the receiver's own `exec`. The engine calls these
// with the receiver as the first argument; the receiver is already known to be
// an object.
//
// One global, __reSpec, carries all six entry points, and the helpers stay
// inside the closure: global lookups scan the global scope's bindings, so every
// prelude global is paid for by every miss in user code.
var __reSpec = (function () {
  var __reBuiltinExec = RegExp.prototype.exec;
  var __reIs = Object.is;
  var __reDefine = Object.defineProperty;
  ...
  function __reHas(str, ch) {
    for (var i = 0; i < str.length; i++) if (str[i] === ch) return true;
    return false;
  }

  // ToLength. Unary plus is ToNumber, which rejects a symbol and a BigInt the way
  // the spec does; Number() would convert the BigInt.
  function __reToLength(v) {
    var n = +v;
    if (n !== n || n <= 0) return 0;
    n = Math.floor(n);
  ...
  function __reToIntegerOrInfinity(v) {
    var n = +v;
    if (n !== n) return 0;
  ...
  function __reAdvance(S, index, unicode) {
    if (!unicode || index + 1 >= S.length) return index + 1;
    var c = S.charCodeAt(index);
    if (c < 0xd800 || c > 0xdbff) return index + 1;
    var d = S.charCodeAt(index + 1);
    return d >= 0xdc00 && d <= 0xdfff ? index + 2 : index + 1;
  }

  function __reDataProp(o, k, v) {
    __reDefine(o, k, { value: v, writable: true, enumerable: true, configurable: true });
  }

  function __reExec(R, S) {
    var exec = R.exec;
    if (typeof exec === "function") {
      var result = exec.call(R, S);
      if (result !== null && typeof result !== "object" && typeof result !== "function") {
        throw new TypeError("exec result must be an object or null");
      }
      return result;
    }
    // RegExpBuiltinExec: a TypeError for anything without [[RegExpMatcher]]
    return __reBuiltinExec.call(R, S);
  }

  function __reSpeciesCtor(O, dflt) {
    var C = O.constructor;
    if (C === undefined) return dflt;
    if (C === null || (typeof C !== "object" && typeof C !== "function")) {
      throw new TypeError("object.constructor is not an object");
    }
  ...
  function __reTest(R, S) {
    return __reExec(R, __reStr(S)) !== null;
  }

  function __reMatch(rx, string) {
    var S = __reStr(string);
    var flags = __reStr(rx.flags);
    if (!__reHas(flags, "g")) return __reExec(rx, S);
    var fullUnicode = __reHas(flags, "u") || __reHas(flags, "v");
    rx.lastIndex = 0;
    var A = [];
    var n = 0;
    for (;;) {
      var result = __reExec(rx, S);
      if (result === null) return n === 0 ? null : A;
      var matchStr = __reStr(result[0]);
      __reDataProp(A, n, matchStr);
      if (matchStr === "") {
        rx.lastIndex = __reAdvance(S, __reToLength(rx.lastIndex), fullUnicode);
      }
      n++;
    }
  }

  function __reSearch(rx, string) {
    var S = __reStr(string);
    var previousLastIndex = rx.lastIndex;
    if (!__reIs(previousLastIndex, 0)) rx.lastIndex = 0;
    var result = __reExec(rx, S);
    var currentLastIndex = rx.lastIndex;
    if (!__reIs(currentLastIndex, previousLastIndex)) rx.lastIndex = previousLastIndex;
    return result === null ? -1 : result.index;
  }

  function __reSplit(rx, string, limit) {
    var S = __reStr(string);
    var C = __reSpeciesCtor(rx, RegExp);
    var flags = __reStr(rx.flags);
    var unicodeMatching = __reHas(flags, "u") || __reHas(flags, "v");
    var newFlags = __reHas(flags, "y") ? flags : flags + "y";
    ...
    var size = S.length;
    if (size === 0) {
      if (__reExec(splitter, S) !== null) return A;
      __reDataProp(A, 0, S);
      return A;
    }
    var p = 0;
    ...
    while (q < size) {
      ...
      var z = __reExec(splitter, S);
      if (z === null) {
        q = __reAdvance(S, q, unicodeMatching);
        continue;
      }
      var e = __reToLength(splitter.lastIndex);
      if (e > size) e = size;
      if (e === p) {
        q = __reAdvance(S, q, unicodeMatching);
        continue;
      }
      __reDataProp(A, lengthA++, S.substring(p, q));
      if (lengthA === lim) return A;
      p = e;
      var numberOfCaptures = __reToLength(z.length) - 1;
      if (numberOfCaptures < 0) numberOfCaptures = 0;
      for (var i = 1; i <= numberOfCaptures; i++) {
        __reDataProp(A, lengthA++, z[i]);
        if (lengthA === lim) return A;
      }
      q = p;
    }
    __reDataProp(A, lengthA, S.substring(p, size));
    return A;
  }

  // GetSubstitution, including the two-digit reference that falls back to one
  // digit when there are not that many captures.
  function __reGetSubstitution(matched, str, position, captures, namedCaptures, replacement) {
    var result = "";
    ...
    while (i < len) {
      ...
      if (d === "$") {
        ...
        if (tailPos < str.length) result += str.substring(tailPos);
        i += 2;
      } else if (d >= "0" && d <= "9") {
        var digitCount = i + 2 < len && replacement[i + 2] >= "0" && replacement[i + 2] <= "9" ? 2 : 1;
        var index = +replacement.substring(i + 1, i + 1 + digitCount);
        ...
        if (index >= 1 && index <= m) {
          ...
          if (capture !== undefined) result += capture;
        } else {
          result += replacement.substring(i, i + 1 + digitCount);
        }
        i += 1 + digitCount;
      } else if (d === "<") {
        var gtPos = replacement.indexOf(">", i);
        if (gtPos === -1 || namedCaptures === undefined) {
          result += "$<";
          i += 2;
        } else {
          var groupName = replacement.substring(i + 2, gtPos);
          var cap = namedCaptures[groupName];
          if (cap !== undefined) result += __reStr(cap);
          i = gtPos + 1;
        }
      } else {
        result += "$";
  ...
  function __reReplace(rx, string, replaceValue) {
    var S = __reStr(string);
    var lengthS = S.length;
    var functionalReplace = typeof replaceValue === "function";
    if (!functionalReplace) replaceValue = __reStr(replaceValue);
    var flags = __reStr(rx.flags);
    var global = __reHas(flags, "g");
    var fullUnicode = false;
    if (global) {
      fullUnicode = __reHas(flags, "u") || __reHas(flags, "v");
      rx.lastIndex = 0;
    }
    var results = [];
    for (;;) {
    ...
    for (var k = 0; k < results.length; k++) {
      var r = results[k];
      var nCaptures = __reToLength(r.length) - 1;
      if (nCaptures < 0) nCaptures = 0;
      var matched = __reStr(r[0]);
      var matchLength = matched.length;
      var position = __reToIntegerOrInfinity(r.index);
      if (position > lengthS) position = lengthS;
      if (position < 0) position = 0;
      var captures = [];
      for (var n = 1; n <= nCaptures; n++) {
        var capN = r[n];
        if (capN !== undefined) capN = __reStr(capN);
        captures[captures.length] = capN;
      }
      var namedCaptures = r.groups;
      var replacement;
      if (functionalReplace) {
        var replacerArgs = [matched];
        for (var ci = 0; ci < captures.length; ci++) replacerArgs[replacerArgs.length] = captures[ci];
        replacerArgs[replacerArgs.length] = position;
        replacerArgs[replacerArgs.length] = S;
        if (namedCaptures !== undefined) replacerArgs[replacerArgs.length] = namedCaptures;
        replacement = __reStr(replaceValue.apply(undefined, replacerArgs));
      } else {
        if (namedCaptures !== undefined) namedCaptures = Object(namedCaptures);
        replacement = __reGetSubstitution(matched, S, position, captures, namedCaptures, replaceValue);
      }
      if (position >= nextSourcePosition) {
        acc += S.substring(nextSourcePosition, position) + replacement;
        nextSourcePosition = position + matchLength;
      }
    }
    if (nextSourcePosition >= lengthS) return acc;
    return acc + S.substring(nextSourcePosition);
  }

  // %RegExpStringIteratorPrototype%. Its state is keyed off the iterator in a
  // WeakMap so nothing about it shows up as a property.
  var __reIterState = new WeakMap();
  var __reIterGet = WeakMap.prototype.get;
  var __reIterSet = WeakMap.prototype.set;
  var __reStringIterProto = Object.create(__iteratorProto);
  __reDefine(__reStringIterProto, "next", {
    value: function next() {
      var st = this !== null && typeof this === "object" ? __reIterGet.call(__reIterState, this) : undefined;
      if (st === undefined) throw new TypeError("next method called on incompatible receiver " + String(this));
      if (st.done) return { value: undefined, done: true };
      var match = __reExec(st.R, st.S);
      if (match === null) {
        st.done = true;
      ...
      if (__reStr(match[0]) === "") {
        st.R.lastIndex = __reAdvance(st.S, __reToLength(st.R.lastIndex), st.fullUnicode);
      }
      return { value: match, done: false };
    },
    writable: true, enumerable: false, configurable: true,
  });
  __reDefine(__reStringIterProto, Symbol.toStringTag, {
    value: "RegExp String Iterator", writable: false, enumerable: false, configurable: true,
  });

  function __reMatchAll(R, string) {
    var S = __reStr(string);
    var C = __reSpeciesCtor(R, RegExp);
    var flags = __reStr(R.flags);
    var matcher = new C(R, flags);
    matcher.lastIndex = __reToLength(R.lastIndex);
    var it = Object.create(__reStringIterProto);
    __reIterSet.call(__reIterState, it, {
      R: matcher, S: S, global: __reHas(flags, "g"),
      fullUnicode: __reHas(flags, "u") || __reHas(flags, "v"), done: false,
    });
    return it;
  }

  return {
    test: __reTest, match: __reMatch, matchAll: __reMatchAll,
    replace: __reReplace, search: __reSearch, split: __reSplit,
  };
})();
...
JSON.stringify = function stringify(value, replacer, space) {
  // Plain data with no replacer or indent serialises natively in one pass;
  // the native answers undefined for any graph that needs ser() below.
  if (replacer === undefined && space === undefined) {
    var fast = __jsonStringifyFast(value);
  ...
  // Objects currently on the path back to the root. A cycle used to recurse
  // until the call-depth guard fired, surfacing as a RangeError where the spec
  // (and node) require a TypeError. Popped on the way out, so the same object
  // appearing twice as SIBLINGS still serialises twice.
  var stack = [];
  function ser(holder, key, v, depth) {
    ...
    var t = typeof v;
    // A BigInt has no JSON representation and node throws. The native serialiser
    // cannot throw (no &mut Interp on that path) so it emits the digits; this
    // wrapper can, and does.
    ...
    if (Array.isArray(v)) {
      if (v.length === 0) { stack.pop(); return "[]"; }
      var parts = [];
      for (var i = 0; i < v.length; i++) {
    ...
    var out = [];
    // Object.keys, NOT for-in: JSON serialises OWN enumerable properties, and
    // for-in walks the prototype chain, so an object created with
    // Object.create({inherited: 1}) serialised its prototype's properties too.
...
// --- %TypedArray%.of / .from -------------------------------------------------
// Statics on %TypedArray% itself, which every concrete constructor inherits:
// `this` is the constructor to build, so BigInt64Array.from works as well.
// `from` takes an iterable or array-like plus an optional map function, like
// Array.from.
(function (TypedArray) {
  var lengthOf = Object.getOwnPropertyDescriptor(TypedArray.prototype, "length").get;
  var tagOf = Object.getOwnPropertyDescriptor(TypedArray.prototype, Symbol.toStringTag).get;
  // TypedArrayCreate: whatever `this` constructs has to BE a typed array, at
  // least as long as asked for. The length getter is the brand check.
  function create(Ctor, len) {
    var out = new Ctor(len);
    if (lengthOf.call(out) < len) throw new TypeError("derived TypedArray constructor created an array which was too small");
    return out;
  }
  // A BigInt view stores through ToBigInt, which rejects a Number, undefined,
  // null and a symbol rather than storing 0 for them.
  function store(out, big, i, v) {
    if (big && (typeof v === "number" || typeof v === "symbol" || v === undefined || v === null)) {
      throw new TypeError("Cannot convert " + (typeof v === "symbol" ? "a Symbol" : String(v)) + " to a BigInt");
    }
    out[i] = v;
  }
  function isBig(out) {
    var tag = tagOf.call(out);
    return tag === "BigInt64Array" || tag === "BigUint64Array";
  }
  function of() {
    var Ctor = this;
    if (!__isConstructor(Ctor)) throw new TypeError("this is not a constructor");
    var out = create(Ctor, arguments.length);
    var big = isBig(out);
    for (var j = 0; j < arguments.length; j++) store(out, big, j, arguments[j]);
    return out;
  }
  // (source, mapFn, thisArg), declared with one parameter so its length is 1
  function from(src) {
    var Ctor = this;
    ...
    var out = create(Ctor, vals.length);
    var big = isBig(out);
    for (var m = 0; m < vals.length; m++) {
      store(out, big, m, mapFn ? mapFn.call(thisArg, vals[m], m) : vals[m]);
    }
    return out;
  }
  __markNotConstructor(of);
  __markNotConstructor(from);
  Object.defineProperty(TypedArray, "of", { value: of, writable: true, enumerable: false, configurable: true });
  Object.defineProperty(TypedArray, "from", { value: from, writable: true, enumerable: false, configurable: true });
