...
var Reflect = {
  ...
  // Reflect does NOT box its argument: Reflect.getPrototypeOf(42) is a
  // TypeError where Object.getPrototypeOf(42) is Number.prototype. Every
  // Reflect entry point requires a real object, and packages test exactly that
  // boundary to tell the two apart.
  ...
  construct: function (target, args, newTarget) {
    ...
    var out = new target(...args);
    // newTarget only changes which .prototype the new object gets; the rest of
    // OrdinaryCreateFromConstructor is what `new target` already did.
    if (arguments.length > 2 && newTarget !== target && out !== null && typeof out === "object") {
      var proto = newTarget.prototype;
      if (proto !== null && typeof proto === "object") Object.setPrototypeOf(out, proto);
    }
    return out;
  },
};
...
RegExp.escape = function escape(S) {
  ...
  for (var i = 0; i < S.length; i++) {
    ...
    if (i === 0 && ((cp >= 48 && cp <= 57) || (cp >= 65 && cp <= 90) || (cp >= 97 && cp <= 122))) {
      ...
      out += "\\x" + (cp < 16 ? "0" : "") + cp.toString(16);
      continue;
    }
    if ("^$\\.*+?()[]{}|/".indexOf(c) >= 0) { out += "\\" + c; continue; }
    // Whitespace and line terminators become their canonical escapes so the
    // result survives being written into a source file.
    if (cp === 9) { out += "\\t"; continue; }
    if (cp === 10) { out += "\\n"; continue; }
    if (cp === 11) { out += "\\v"; continue; }
    if (cp === 12) { out += "\\f"; continue; }
    if (cp === 13) { out += "\\r"; continue; }
...
function btoa(data) {
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    if (c > 0xff) {
      throw new TypeError("btoa: string contains a character outside of the Latin1 range");
    }
  }
...
// nested blocks would. `using` syntax is not parsed here yet; the class is the
// half real code and the test suites reach for directly.
(function () {
