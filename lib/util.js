// node:util — the commonly-used surface, in the ES5 subset milojs supports.
"use strict";

var _utilErr = require('_errors');

function inherits(ctor, superCtor) {
  // Both have to be constructors. These were taking anything and assigning a
  // prototype onto it, so inherits(1, 1) silently did nothing.
  if (typeof ctor !== 'function') throw _utilErr.ERR_INVALID_ARG_TYPE('ctor', ['Function'], ctor);
  if (typeof superCtor !== 'function') {
    throw _utilErr.ERR_INVALID_ARG_TYPE('superCtor', ['Function'], superCtor);
  }
  if (superCtor.prototype === undefined) {
    throw _utilErr.ERR_INVALID_ARG_TYPE('superCtor.prototype', ['Object'], superCtor.prototype);
  }
  ctor.super_ = superCtor;
  var proto = {};
  for (var k in superCtor.prototype) {
    proto[k] = superCtor.prototype[k];
  }
  proto.constructor = ctor;
  ctor.prototype = proto;
}

function isArray(x) {
  return x !== null && typeof x === "object" && typeof x.length === "number" && typeof x.push === "undefined" ? false : Array.isArray ? Array.isArray(x) : false;
}

// Delegates to the engine's own renderer (the one console.log uses), so
// util.inspect and console.log can never drift apart. The JS fallback below is
// kept for an embedder that boots without the native — it is bun-shaped and
// does NOT reproduce node's line-breaking or array column grouping.
function inspect(v, optsOrShowHidden, legacyDepth, legacyColors) {
  // node's second parameter is an OPTIONS object (or the legacy showHidden
  // boolean, with depth and colors following it). This function's own recursion
  // passes an array — its seen list — which is how the two are told apart.
  // Taking every second argument as the seen list is what made
  // util.inspect(x, { depth: 2 }) throw "indexOf is not a function".
  if (!Array.isArray(optsOrShowHidden)) {
    var opts = optsOrShowHidden;
    if (typeof optsOrShowHidden === "boolean") {
      opts = { showHidden: optsOrShowHidden, depth: legacyDepth, colors: legacyColors };
    }
    if (typeof __inspect === "function") {
      return opts === undefined || opts === null ? __inspect(v) : __inspect(v, opts);
    }
    return inspectFallback(v, []);
  }
  return inspectFallback(v, optsOrShowHidden);
}

// Kept for an embedder that boots without the native renderer — it is
// bun-shaped and does NOT reproduce node's line-breaking or array column
// grouping, and it ignores the options entirely.
function inspectFallback(v, seen) {
  if (v === null) {
    return "null";
  }
  if (v === undefined) {
    return "undefined";
  }
  var t = typeof v;
  if (t === "string") {
    return "'" + v + "'";
  }
  if (t === "bigint") {
    return "" + v + "n";
  }
  if (t === "number" || t === "boolean") {
    return "" + v;
  }
  if (t === "symbol") {
    return v.toString();
  }
  if (t === "function") {
    return v.name ? "[Function: " + v.name + "]" : "[Function (anonymous)]";
  }
  if (v instanceof Date) {
    return v.toISOString();
  }
  if (v instanceof Error) {
    return v.stack || v.name + ": " + v.message;
  }
  seen = seen || [];
  if (seen.indexOf(v) >= 0) {
    return "[Circular]";
  }
  seen.push(v);
  var out;
  if (Array.isArray(v)) {
    if (v.length === 0) {
      out = "[]";
    } else {
      var parts = [];
      for (var i = 0; i < v.length; i++) {
        parts.push(inspectFallback(v[i], seen));
      }
      out = "[ " + parts.join(", ") + " ]";
    }
  } else {
    var keys = Object.keys(v);
    if (keys.length === 0) {
      out = "{}";
    } else {
      var kp = [];
      for (var j = 0; j < keys.length; j++) {
        var key = keys[j];
        // bare identifier keys unquoted, everything else single-quoted
        var ident = /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(key);
        var kd = ident ? key : "'" + key + "'";
        kp.push(kd + ": " + inspectFallback(v[key], seen));
      }
      out = "{ " + kp.join(", ") + " }";
    }
  }
  seen.pop();
  return out;
}

// util.format with %s %d %i %j %% — the subset real code uses.
function format(fmt) {
  var rest = [];
  for (var i = 1; i < arguments.length; i++) {
    rest.push(arguments[i]);
  }
  if (typeof fmt !== "string") {
    var parts = [inspect(fmt)];
    for (var k = 0; k < rest.length; k++) {
      parts.push(inspect(rest[k]));
    }
    return parts.join(" ");
  }
  var out = "";
  var ri = 0;
  var idx = 0;
  while (idx < fmt.length) {
    var c = fmt.charAt(idx);
    if (c === "%" && idx + 1 < fmt.length) {
      var n = fmt.charAt(idx + 1);
      if (n === "%") {
        out = out + "%";
        idx = idx + 2;
        continue;
      }
      if (n === "s" || n === "d" || n === "i" || n === "j") {
        if (ri < rest.length) {
          var a = rest[ri];
          ri = ri + 1;
          if (n === "s") {
            out = out + (typeof a === "string" ? a : inspect(a));
          } else if (n === "j") {
            out = out + JSON.stringify(a);
          } else {
            out = out + Math.trunc(Number(a));
          }
        } else {
          out = out + "%" + n;
        }
        idx = idx + 2;
        continue;
      }
    }
    out = out + c;
    idx = idx + 1;
  }
  for (var r = ri; r < rest.length; r++) {
    out = out + " " + inspect(rest[r]);
  }
  return out;
}

function deprecate(fn, msg) {
  return fn;
}

// Node's callback->Promise adapter. Honors util.promisify.custom, which is how
// child_process.exec advertises its {stdout,stderr} shape instead of the plain
// single-value callback contract.
function promisify(original) {
  if (typeof original !== 'function') {
    throw _utilErr.ERR_INVALID_ARG_TYPE('original', ['Function'], original);
  }
  if (typeof original !== 'function') {
    throw new TypeError('The "original" argument must be of type function');
  }
  var custom = original[promisify.custom];
  if (custom) {
    if (typeof custom !== 'function') {
      throw new TypeError('The "promisify.custom" property must be of type function');
    }
    return custom;
  }
  function fn() {
    var args = Array.prototype.slice.call(arguments);
    var self = this;
    return new Promise(function (resolve, reject) {
      args.push(function (err, value) {
        if (err) { reject(err); } else { resolve(value); }
      });
      original.apply(self, args);
    });
  }
  return fn;
}
promisify.custom = '__util_promisify_custom__';

exports.promisify = promisify;
// The web-standard classes node re-exports from util. They exist as globals here
// already; only the re-export was missing, and `const { TextEncoder } =
// require("util")` then reads undefined, which surfaces as
// "value is not a constructor" at the `new`, naming nothing useful.
//
// Only what node actually re-exports: AbortController and AbortSignal are
// globals but are NOT on util, and adding them would be a divergence in the
// other direction (`typeof require("util").AbortController` is "undefined" in
// node).
if (typeof TextEncoder !== "undefined") exports.TextEncoder = TextEncoder;
if (typeof TextDecoder !== "undefined") exports.TextDecoder = TextDecoder;
exports.inherits = inherits;
exports.inspect = inspect;
exports.format = format;
exports.deprecate = deprecate;
exports.isArray = isArray;

// util.types — type predicates some deps use for defensive checks. Backed by
// instanceof (which recognizes the built-in exotics) where reliable; the
// undetectable ones (Proxy, async-function) report false rather than lie.
function _tag(v) { return Object.prototype.toString.call(v).slice(8, -1); }

exports.types = {
  isDate: function (v) { return v instanceof Date; },
  isRegExp: function (v) { return v instanceof RegExp; },
  isMap: function (v) { return v instanceof Map; },
  isSet: function (v) { return v instanceof Set; },
  isPromise: function (v) { return v instanceof Promise; },
  isNativeError: function (v) { return v instanceof Error; },
  isArrayBuffer: function (v) { return v instanceof ArrayBuffer; },
  isAnyArrayBuffer: function (v) { return v instanceof ArrayBuffer; },
  isTypedArray: function (v) {
    // the runtime polyfills typed arrays as plain arrays tagged _isTypedArray;
    // the engine has real ones that answer instanceof
    if (v && v._isTypedArray) return true;
    return v instanceof Uint8Array || v instanceof Int8Array || v instanceof Uint8ClampedArray ||
      v instanceof Uint16Array || v instanceof Int16Array || v instanceof Uint32Array ||
      v instanceof Int32Array || v instanceof Float32Array || v instanceof Float64Array;
  },
  // These four returned a constant `false`, which made them LIE about features
  // this engine has: an async function, a generator function and a boxed Number
  // are all real here, and a caller dispatching on util.types took the wrong
  // branch silently. They read the type tag now, which reports
  // [object AsyncFunction] / [object GeneratorFunction] / [object Generator].
  isAsyncFunction: function (v) { return _tag(v) === "AsyncFunction" || _tag(v) === "AsyncGeneratorFunction"; },
  isGeneratorFunction: function (v) { return _tag(v) === "GeneratorFunction" || _tag(v) === "AsyncGeneratorFunction"; },
  isGeneratorObject: function (v) { return _tag(v) === "Generator" || _tag(v) === "AsyncGenerator"; },
  // Written as a real check even though it can only answer false today: this
  // engine has no wrapper objects at all (`new Number(1)` returns the PRIMITIVE
  // 1, so typeof is "number" and instanceof Number is false), so there is
  // nothing boxed to find. The check is here rather than a `return false`
  // because the day wrappers exist it becomes correct on its own, and because
  // the reason is a property of the engine that a reader can verify, not an
  // assertion they have to trust. The missing wrappers are in the backlog.
  isBoxedPrimitive: function (v) {
    if (v === null || typeof v !== "object") return false;
    var t = _tag(v);
    return t === "Number" || t === "String" || t === "Boolean" || t === "Symbol" || t === "BigInt";
  },
  // Still a constant, and still `false`, but for a reason that is a property of
  // the engine rather than a guess: a Proxy here is indistinguishable from its
  // target through any JS-visible channel (the type tag reflects the target, as
  // it does in node), so there is nothing to read. node answers true using an
  // internal slot this engine does not expose. Tracked in the backlog.
  isProxy: function () { return false; },
};

// util.getCallSites (Node 22+). The engine exposes raw frames through
// __callFrames; this reshapes them into the objects node documents. Line and
// column are 0 when a frame does not carry them rather than invented, since a
// wrong location is worse than an obviously absent one.
exports.getCallSites = function getCallSites(frameCountOrOptions, maybeOptions) {
  var limit = typeof frameCountOrOptions === 'number' ? frameCountOrOptions : 10;
  var raw = [];
  try { raw = __callFrames(); } catch (e) { raw = []; }
  var out = [];
  for (var i = 0; i < raw.length && out.length < limit; i++) {
    var f = raw[i];
    if (typeof f === 'string') {
      out.push({ functionName: f, scriptId: '0', scriptName: '', lineNumber: 0, columnNumber: 0, column: 0 });
    } else if (f && typeof f === 'object') {
      out.push({
        functionName: f.functionName || f.name || '',
        scriptId: String(f.scriptId || 0),
        scriptName: f.scriptName || f.file || '',
        lineNumber: f.lineNumber || f.line || 0,
        columnNumber: f.columnNumber || f.column || 0,
        column: f.columnNumber || f.column || 0,
      });
    }
  }
  return out;
};

// util.debuglog: a logger that is a no-op unless its section is named in
// NODE_DEBUG. The returned function carries `.enabled` so a caller can skip
// building an expensive message at all.
exports.debuglog = function debuglog(section, cb) {
  var env = (typeof process !== 'undefined' && process.env && process.env.NODE_DEBUG) || '';
  var enabled = env.split(',').some(function (s) { return s.trim() === section || s.trim() === '*'; });
  var fn = enabled
    ? function () {
        var args = Array.prototype.slice.call(arguments);
        console.error(section.toUpperCase() + ' ' + (process.pid || 0) + ': ' + exports.format.apply(null, args));
      }
    : function () {};
  Object.defineProperty(fn, 'enabled', { value: enabled, enumerable: true, configurable: true });
  if (typeof cb === 'function') cb(fn);
  return fn;
};
exports.debug = exports.debuglog;

// The inverse of promisify: a promise-returning function presented with a
// node-style (err, value) callback.
exports.callbackify = function callbackify(original) {
  if (typeof original !== 'function') {
    throw new TypeError('The "original" argument must be of type function');
  }
  function callbackified() {
    var args = Array.prototype.slice.call(arguments);
    var cb = args.pop();
    if (typeof cb !== 'function') {
      throw new TypeError('The last argument must be of type function');
    }
    var self = this;
    original.apply(self, args).then(
      function (ret) { process.nextTick(function () { cb.call(self, null, ret); }); },
      function (rej) {
        // A rejection with a FALSY reason still has to reach the callback as an
        // error, or `if (err)` misses it entirely.
        var reason = rej;
        if (!reason) {
          reason = new Error('Promise was rejected with a falsy value');
          reason.reason = rej;
        }
        process.nextTick(function () { cb.call(self, reason); });
      }
    );
  }
  Object.setPrototypeOf(callbackified, Object.getPrototypeOf(original));
  return callbackified;
};

// A minimal ANSI wrapper. An unknown format is a TypeError rather than silently
// unstyled text, which is how a typo gets found.
var STYLES = {
  reset: [0, 0], bold: [1, 22], italic: [3, 23], underline: [4, 24], inverse: [7, 27],
  red: [31, 39], green: [32, 39], yellow: [33, 39], blue: [34, 39], magenta: [35, 39],
  cyan: [36, 39], white: [37, 39], gray: [90, 39], grey: [90, 39],
};
exports.styleText = function styleText(format, text, options) {
  if (typeof text !== 'string') {
    throw new TypeError('The "text" argument must be of type string');
  }
  var list = Array.isArray(format) ? format : [format];
  // The format is validated even when nothing will be styled, so a typo is
  // caught on a pipe as well as on a terminal.
  for (var v = 0; v < list.length; v++) {
    if (!STYLES[list[v]]) {
      throw new TypeError('The "format" argument must be one of: ' + Object.keys(STYLES).join(', '));
    }
  }
  // Node emits no escapes when the destination cannot show them, so piping the
  // output gives plain text rather than a screenful of control codes.
  var stream = (options && options.stream) || (typeof process !== 'undefined' ? process.stdout : null);
  if (!stream || !stream.isTTY) return text;
  var out = text;
  for (var i = list.length - 1; i >= 0; i--) {
    var s = STYLES[list[i]];
    if (!s) throw new TypeError('The "format" argument must be one of: ' + Object.keys(STYLES).join(', '));
    out = '\u001b[' + s[0] + 'm' + out + '\u001b[' + s[1] + 'm';
  }
  return out;
};
