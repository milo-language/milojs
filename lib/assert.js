...
class AssertionError extends Error {
  constructor(options) {
    ...
    this.generatedMessage = opts.generatedMessage !== undefined ? opts.generatedMessage : !opts.message;
  }
}

// The user's message and the generated one are separate arguments, because
// `generatedMessage` has to say which of the two was used and `message ||
// fallback` throws that away. node's tests read the flag directly.
function failWith(userMessage, generatedText, actual, expected, operator) {
  var generated = userMessage === undefined || userMessage === null;
  throw new AssertionError({
    message: generated ? generatedText : userMessage,
    actual: actual,
    expected: expected,
    operator: operator,
...
function deepEq(a, b, strict, seen) {
  ...
  if (a instanceof Date || b instanceof Date) {
    ...
    // An object that merely INHERITS Date.prototype has no date value, and
    // getTime throws on it rather than answering. Two such fakes are compared
    // structurally, like the plain objects they are; a fake and a real Date are
    // never equal. Calling getTime unguarded turned this into a TypeError that
    // escaped the assertion entirely.
    var at, bt, aFake = false, bFake = false;
    try { at = a.getTime(); } catch (e) { aFake = true; }
    try { bt = b.getTime(); } catch (e) { bFake = true; }
    if (aFake !== bFake) return false;
    if (!aFake) return at === bt;
  }
...
function matchError(err, expected) {
  ...
  if (expected instanceof RegExp) {
    ...
    // The brand, not instanceof: an error thrown inside a vm context is made
    // from that context's Error and is no instance of this one.
    var isError = err instanceof Error || Object.prototype.toString.call(err) === '[object Error]';
    var subject = isError ? String(err) : String(err && err.message);
...
assert.doesNotThrow = function (fn, expected, message) {
  ...
  try {
    ...
    failWith(message ? message + ': ' + 'Got unwanted exception: ' + (e && e.message) : undefined,
             'Got unwanted exception: ' + (e && e.message), e, expected, 'doesNotThrow');
  }
};
