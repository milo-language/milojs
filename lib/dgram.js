...
//
// PAYLOADS cross as Uint8Array. They cannot cross as strings: a milojs string is
// UTF-8, so every byte >= 0x80 was re-encoded on the way out and a 6-byte
// datagram arrived at a node peer as 10 bytes.
"use strict";
...
var pumpAll = {
  _serveOnce: function () {
    ...
    for (var i = pending.length - 1; i >= 0; i--) {
      ...
      did = true;
      s.emit('message', require('buffer').Buffer.from(msg.data), {
...
Socket.prototype.send = function (buf, offsetOrPort, lengthOrAddress, portOrCb, addressOrCb, maybeCb) {
  ...
  var payload;
  if (typeof buf === 'string') payload = Buffer.from(buf, 'utf8');
  else if (Array.isArray(buf)) payload = Buffer.concat(buf.map(function (b) {
    return typeof b === 'string' ? Buffer.from(b) : b;
  }));
  else if (buf instanceof Uint8Array) payload = Buffer.from(buf);
  else throw _err.ERR_INVALID_ARG_TYPE('buffer', ['Buffer', 'TypedArray', 'DataView', 'string'], buf);

  if (length !== undefined) payload = payload.subarray(offset, offset + length);
  else if (offset) payload = payload.subarray(offset);
...
function createSocket(typeOrOptions, listener) {
  // The socket type is udp4 or udp6 and nothing else. node reports a bad one as
  // ERR_SOCKET_BAD_TYPE before constructing anything, so a typo fails at the
  // call rather than at the first send.
  var type = typeOrOptions;
  if (typeOrOptions !== null && typeof typeOrOptions === 'object') type = typeOrOptions.type;
  if (type !== 'udp4' && type !== 'udp6') {
    throw require('_errors').codedError(TypeError, 'ERR_SOCKET_BAD_TYPE',
