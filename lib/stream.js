...
Writable.prototype._writeNow = function (chunk, encoding, cb) {
  var self = this;
  // An error handed to the write callback is how a _write implementation reports
  // failure, and node turns it into an 'error' event on the stream (and destroys
  // it). milojs passed the error to the caller's callback and stopped there, so
  // a program waiting on 'error' — which is every stream test and every
  // pipeline() — waited for an event that was never coming.
  //
  // The user callback runs FIRST: node's own tests assert inside the 'error'
  // handler that the write callback already ran.
  var done = function (err) {
    if (cb) cb(err);
    if (err) self._failWrite(err);
  };
  if (this._writeImpl) {
...
Writable.prototype._failWrite = function (err) {
