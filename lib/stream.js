...
Readable.prototype.push = function (chunk) {
  // A push answers the outstanding _read request, whether it came back on this
  // tick or ten ticks later, so the next request is allowed once it lands.
  this._reading = false;
  if (chunk === null) {
    this._ended = true;
...
// Ask the producer for more. This half of the contract was missing entirely:
// `options.read` was stored and never called, so a stream that fills ON DEMAND
// — the shape every node example uses — emitted nothing at all and the program
// sat waiting for data that was never requested. 162 node-compat cases hung on
// it, scored as "timeout" with nothing naming the cause.
Readable.prototype._maybeRead = function () {
  if (this._ended || this._reading || typeof this._read !== 'function') return;
  if (this.destroyed || this._flowing === false) return;
  // Above the mark there is already enough buffered; asking again is what turns
  // a fast producer into an unbounded one.
  if (this._chunks.length >= this._highWaterMark) return;
  this._reading = true;
...
Readable.prototype._drain = function () {
  ...
  setTimeout(function () {
    while (self._chunks.length > 0 && self._flowing !== false) {
    ...
    // Drained everything on hand: pull again. A producer that pushes on a later
    // tick gets its next request from the push that answers this one.
    if (self._flowing) self._maybeRead();
  }, 0);
};
