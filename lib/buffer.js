...
// node's rule, which distinguishes two different failures that were both being
// reported as ERR_OUT_OF_RANGE:
//   - a buffer too small to hold the type AT ALL is ERR_BUFFER_OUT_OF_BOUNDS
//   - an offset past the end of a big-enough buffer is ERR_OUT_OF_RANGE
// A four-byte buffer asked for a double is the first; the same buffer asked for
// a byte at index 9 is the second.
function boundsError(value, length, type) {
  if (Math.floor(value) !== value) {
    if (typeof value !== 'number') throw invalidArgType(type || 'offset', 'of type number', value);
...
function checkBounds(buf, offset, size) {
  ...
  // The LAST byte the access touches is offset + size - 1, not offset + size.
  // Reading four bytes at offset 0 of a four-byte buffer is legal, and the
  // reported maximum offset is length - size.
  if (buf[offset] === undefined || buf[offset + size - 1] === undefined) {
    boundsError(offset, buf.length - size);
  }
}
...
// The representable range for an integer accessor, derived from its own name so
// a new one cannot be added without one. Float and double accessors have no
// such range and pass null.
function intRangeOf(name, size) {
  if (name.indexOf('Float') >= 0 || name.indexOf('Double') >= 0) return null;
  if (name.indexOf('BigInt') >= 0 || name.indexOf('BigUInt') >= 0) return null;
  var bits = size * 8;
  if (name.indexOf('UInt') >= 0) return { min: 0, max: Math.pow(2, bits) - 1 };
  return { min: -Math.pow(2, bits - 1), max: Math.pow(2, bits - 1) - 1 };
}

function defWrite(name, size, fn) {
  var range = intRangeOf(name, size);
  Buffer.prototype[name] = function (value, offset) {
    var o = offset === undefined ? 0 : offset;
    // Range BEFORE bounds, which is node's order: a value that cannot be
    // represented is reported as such even when the offset is also wrong.
    if (range) {
      if (typeof value !== 'number') throw invalidArgType('value', 'of type number', value);
      if (value < range.min || value > range.max || Math.floor(value) !== value) {
        throw outOfRange('value', '>= ' + range.min + ' and <= ' + range.max, value);
      }
    }
    checkBounds(this, o, size);
    fn(this, value, o);
...
// The variable-length accessors take a byteLength, and node validates it FIRST:
// 1 to 6 bytes, because beyond six the value stops being exactly representable
// as a double. It was going straight into the bounds check, so a byteLength of
// 0 or 7 was reported as an offset problem, or as a bounds error, rather than
// as the argument that was actually wrong.
function checkByteLength(len) {
  if (typeof len !== 'number') throw invalidArgType('byteLength', 'of type number', len);
  if (Math.floor(len) !== len) throw outOfRange('byteLength', 'an integer', len);
  if (len < 1 || len > 6) throw outOfRange('byteLength', '>= 1 and <= 6', len);
}

Buffer.prototype.readUIntBE = function (o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _rdU(this, o || 0, len, true); };
Buffer.prototype.readUIntLE = function (o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _rdU(this, o || 0, len, false); };
Buffer.prototype.readIntBE = function (o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _sgn(_rdU(this, o || 0, len, true), len * 8); };
Buffer.prototype.readIntLE = function (o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _sgn(_rdU(this, o || 0, len, false), len * 8); };
Buffer.prototype.writeUIntBE = function (v, o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _wrU(this, v, o || 0, len, true); };
Buffer.prototype.writeUIntLE = function (v, o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _wrU(this, v, o || 0, len, false); };
Buffer.prototype.writeIntBE = function (v, o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _wrU(this, v < 0 ? v + Math.pow(2, len * 8) : v, o || 0, len, true); };
Buffer.prototype.writeIntLE = function (v, o, len) { checkByteLength(len); checkBounds(this, o || 0, len); return _wrU(this, v < 0 ? v + Math.pow(2, len * 8) : v, o || 0, len, false); };
