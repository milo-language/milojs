...
// node's inspect escapes C0 controls as \xNN (with the short forms for the
// ones that have them), which is what its error messages carry.
var SHORT_ESCAPE = { 8: '\\b', 9: '\\t', 10: '\\n', 12: '\\f', 13: '\\r' };
function escapeControl(text) {
  var out = '';
  for (var i = 0; i < text.length; i++) {
    var c = text.charCodeAt(i);
    if (c < 0x20 || c === 0x7f) {
      out += SHORT_ESCAPE[c] || ('\\x' + (c < 16 ? '0' : '') + c.toString(16));
    } else {
      out += text.charAt(i);
    }
  }
...
function receivedText(value) {
  ...
  var inspected;
  // A control character is ESCAPED in node's inspect output, and these messages
  // are compared whole: a path carrying a NUL printed the raw byte here, so the
  // message differed from node's by an invisible character.
  if (typeof value === 'string') inspected = "'" + escapeControl(value) + "'";
...
function invalidArgValue(name, value, reason) {
  return coded(TypeError, 'ERR_INVALID_ARG_VALUE',
    ...
    (typeof value === 'string' ? "'" + escapeControl(value) + "'" : String(value)));
}

// The PROPERTY form: node words an invalid option field as "The property
// 'options.mode' ..." where a positional argument is "The argument 'mode' ...".
function invalidArgValueProp(name, value, reason) {
  return coded(TypeError, 'ERR_INVALID_ARG_VALUE',
    "The property '" + name + "' " + (reason || 'is invalid') + '. Received ' +
    (typeof value === 'string' ? "'" + escapeControl(value) + "'" : String(value)));
}
...
module.exports = {
  ...
  ERR_INVALID_ARG_VALUE_PROP: invalidArgValueProp,
