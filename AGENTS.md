...
| `tools/gen-node-compat.mjs` | writes `docs/node-compat.md` by diffing each `node:<module>`'s exports against a committed snapshot of what node and bun export, joined with the sweep's per-area rate. `--check` gates; `--refresh` re-records the snapshot. |
